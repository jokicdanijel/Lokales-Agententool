...
async def _route_register() -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            sp = {"src": "openweb", "dst": "archivp", "kind": "ROUTE", "body": payload, "strict": True}
...
async def ping() -> dict[str, Any]:
    """Ping endpoint."""
    return {"ok": True, "pong": "openweb", "ts": _now(), "strict": True}
...
async def call(body: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {"action": action, "data": _redact(data)}

    # Write CALL safepoint to OpenA2
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            sp = {"src": "openweb", "dst": "archivp", "kind": "CALL", "body": payload, "strict": True}
    ...
    return {"ok": True, "echo": data.get("text", ""), "strict": True}


if __name__ == "__main__":
    import uvicorn

    uvicorn.run("src.services.openwebui.service:app", host="127.0.0.1", port=PORT, reload=False, access_log=False)
