...
class TokenValidationResult(BaseModel):
    ...
    claims: TokenClaims | None = Field(None, description="Token claims if valid")
...
def create_token(
    ...
    # Encode token
    token = jwt.encode(claims.dict(), private_key, algorithm=JWTConfig.ALGORITHM)
...
def verify_token(token: str) -> TokenValidationResult:
    ...
    try:
        ...
        return TokenValidationResult(valid=False, error=str(e), error_type="KEY_LOAD_ERROR")

    try:
        # Decode and verify
        payload = jwt.decode(
            token, public_key, algorithms=[JWTConfig.ALGORITHM], audience=JWTConfig.AUDIENCE, issuer=JWTConfig.ISSUER
        )
        ...
        return TokenValidationResult(valid=False, error="Token has expired", error_type="EXPIRED")

    except jwt.InvalidSignatureError:
        return TokenValidationResult(valid=False, error="Token signature is invalid", error_type="INVALID_SIGNATURE")

    except jwt.InvalidAudienceError:
        return TokenValidationResult(valid=False, error="Token audience is invalid", error_type="INVALID_AUDIENCE")

    except jwt.InvalidIssuerError:
        return TokenValidationResult(valid=False, error="Token issuer is invalid", error_type="INVALID_ISSUER")

    except jwt.DecodeError as e:
        return TokenValidationResult(valid=False, error=f"Token decode error: {e!s}", error_type="DECODE_ERROR")

    except Exception as e:
        return TokenValidationResult(valid=False, error=str(e), error_type="UNKNOWN_ERROR")


def refresh_token(token: str, agent_id: str) -> str | None:
    """
    ...
    return create_token(agent_id=agent_id, scope=claims.scope, permissions=claims.permissions)
...
async def verify_token_from_header(authorization_header: str) -> TokenValidationResult:
    ...
    if not authorization_header:
        return TokenValidationResult(valid=False, error="Missing Authorization header", error_type="MISSING_HEADER")
...
async def demo():
    ...
    token = create_token(agent_id="opena1", scope="invoke", permissions=["read", "write"])
