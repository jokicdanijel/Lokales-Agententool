...
app = FastAPI(title="opena6_Email", version="1.0.0", description="Email Integration Agent (SMTP/IMAP)")
...
TEMPLATES = {
    ...
    "notification": {"subject": "New notification: {{title}}", "body": "You have a new message: {{message}}"},
}
...
async def send_email(req: EmailSendRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive({"op": "SEND_EMAIL", "to": req.to, "subject": req.subject, "bytes": len(req.body)})
...
async def send_template_email(req: EmailTemplateRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive({"op": "TEMPLATE_RENDER", "template": req.template_name, "variables": len(req.variables)})
...
async def read_emails(req: EmailReadRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive({"op": "READ_EMAILS", "folder": req.folder, "limit": req.limit})
...
async def list_templates(authorization: str = Header(None)):
    ...
    return {"strict": True, "templates": templates, "count": len(templates), "ts": datetime.utcnow().isoformat() + "Z"}
