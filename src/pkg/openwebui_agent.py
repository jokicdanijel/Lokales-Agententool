...
async def health() -> HealthResponse:
    ...
    return HealthResponse(status="ok", ts=datetime.utcnow().isoformat() + "Z", port=12347)
    ...
    try:
        async with httpx.AsyncClient(timeout=OPENWEBUI_TIMEOUT) as client:
            ...
            response = await client.post(
                ...
                json={"message": req.message, "model": req.model, "temperature": req.temperature},
        ...
        raise HTTPException(status_code=502, detail=f"OpenWebUI error: {e!s}")
...
async def command(payload: dict) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=OPENWEBUI_TIMEOUT) as client:
            response = await client.post(
                f"{OPENWEBUI_URL}/api/command", json=payload, headers={"Content-Type": "application/json"}
            )
            response.raise_for_status()
