...
class MailAgent(AgentBase):
    ...
    def __init__(self, memory_system=None):
        super().__init__(agent_id="mail_agent", capabilities=[AgentCapability.EMAIL], memory_system=memory_system)

        # Agent-spezifische Konfiguration
        self.smtp_config = {"host": "smtp.example.com", "port": 587, "use_tls": True}

        self.imap_config = {"host": "imap.example.com", "port": 993, "use_ssl": True}
    ...
    async def initialize(self) -> None:
        ...
        await self.store_memory("last_init", datetime.now(UTC).isoformat())
    ...
    async def execute(self, command: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            if command == "send_mail":
                ...
                result = {"status": "error", "error": f"Unknown command: {command}", "data": None}
    ...
    async def _send_mail(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for field in required:
            if field not in params:
                return {"status": "error", "error": f"Missing required field: {field}", "data": None}

        # TODO: Tatsächliches SMTP-Senden
        logger.info(f"Sending mail to {params['to']}: {params['subject']}")

        # Simulated send
        mail_id = f"mail_{datetime.now(UTC).timestamp()}"

        # Im Memory speichern
        await self.store_memory(
            f"sent_{mail_id}",
            {"to": params["to"], "subject": params["subject"], "sent_at": datetime.now(UTC).isoformat()},
        )
    ...
    async def _list_inbox(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"status": "success", "data": {"mails": mails, "count": len(mails)}, "error": None}
    ...
    async def _read_mail(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not mail_id:
            return {"status": "error", "error": "Missing required field: mail_id", "data": None}
        ...
        return {"status": "success", "data": mail_content, "error": None}
