...
class AgentManager:
    ...
    async def execute_command(
        ...
        if agent_id:
            if agent_id not in self.agents:
                return {"status": "error", "error": f"Agent {agent_id} not found", "data": None}
            ...
            try:
                ...
                return {"status": "error", "error": str(e), "data": None, "agent_id": agent_id}

        # Option 2: Auto-Routing via Capability
        if capability:
            candidates = [
                a for a in self.agents.values() if a.supports_capability(capability) and a.status == AgentStatus.READY
            ]

            if not candidates:
                return {"status": "error", "error": f"No agent found with capability {capability.value}", "data": None}
            ...
            try:
                ...
                return {"status": "error", "error": str(e), "data": None, "agent_id": agent.agent_id}

        # Keine Routing-Info
        return {"status": "error", "error": "Must provide either agent_id or capability for routing", "data": None}
    ...
    async def find_agents_by_capability(self, capability: AgentCapability) -> list[str]:
        ...
        Returns:
            Liste von Agent-IDs
        """
        return [agent_id for agent_id, agent in self.agents.items() if agent.supports_capability(capability)]
