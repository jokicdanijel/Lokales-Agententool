...
class BaseAgentConfig(BaseSettings):
    ...
    # Authentication
    bearer_token: str = Field(default="", alias="BEARER_TOKEN", description="Bearer token for authentication")

    # Base directory
    base_dir: Path = Field(default_factory=lambda: Path.cwd(), description="Base directory for the service")

    # Service URLs
    opena1_url: str = Field(default="http://127.0.0.1:12344", alias="OPENA1_URL", description="opena1 coordinator URL")
    opena2_url: str = Field(default="http://127.0.0.1:12345", alias="OPENA2_URL", description="opena2 archivator URL")
    opena20_url: str = Field(default="http://127.0.0.1:12349", alias="OPENA20_URL", description="opena20 dashboard URL")
    ...
    def get_logging_config(self) -> dict:
        ...
        return {
            ...
            "handlers": {
                "console": {"class": "logging.StreamHandler", "formatter": "standard", "level": self.log_level}
            },
            "loggers": {self.service_name: {"handlers": ["console"], "level": self.log_level, "propagate": False}},
...
__all__ = ["PortPolicy", "BaseAgentConfig", "AgentInfo"]
