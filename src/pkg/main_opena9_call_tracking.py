...
app = FastAPI(title="opena9_CallTracking", version="1.0.0", description="Call Analytics & CRM Integration")
...
async def get_transcription(req: TranscriptionRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive({"op": "GET_TRANSCRIPTION", "call_id": req.call_id, "length": len(transcription)})
...
async def analyze_sentiment(req: SentimentAnalysisRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive(
            {"op": "ANALYZE_SENTIMENT", "call_id": req.call_id, "sentiment": sentiment, "confidence": confidence}
        )
...
async def sync_crm(req: CRMSyncRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive({"op": "CRM_SYNC", "contact_id": req.contact_id, "fields": len(req.call_data)})
...
async def get_history(authorization: str = Header(None), limit: int = 10):
    ...
    try:
        ...
        return {"strict": True, "calls": history, "count": len(history), "ts": datetime.utcnow().isoformat() + "Z"}
    except Exception as e:
