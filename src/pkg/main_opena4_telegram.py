...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_DIR / "opena4_telegram.log"), logging.StreamHandler()],
)
...
async def _archive_write(payload: dict[str, Any]) -> bool:
    ...
    try:
        data = {"src": "opena4_telegram", "dst": "opena2", "kind": "MESSAGE", "payload": payload}
...
async def _send_telegram_message(chat_id: int, text: str) -> bool:
    ...
    try:
        ...
        req = urllib.request.Request(
            url, data=data.encode("utf-8"), headers={"Content-Type": "application/json"}, method="POST"
        )
        with urllib.request.urlopen(req, timeout=5) as r:
            result = json.loads(r.read().decode())
...
async def _route_to_finance(command: str, args: list[str], user_id: int) -> str:
    ...
    try:
        ...
        if command == "balance":
            req = urllib.request.Request(
                f"{OPENA_FINANCE}/dashboard", headers={"Authorization": f"Bearer {DASHBOARD_TOKEN}"}, method="GET"
            )
            with urllib.request.urlopen(req, timeout=5) as r:
                result = json.loads(r.read().decode())
            ...
            req = urllib.request.Request(
                f"{OPENA_FINANCE}/accounts", headers={"Authorization": f"Bearer {DASHBOARD_TOKEN}"}, method="GET"
            )
            with urllib.request.urlopen(req, timeout=5) as r:
                result = json.loads(r.read().decode())
                accounts = result.get("accounts", [])
        ...
        return f"❌ Finance service error: {e!s}"
...
async def telegram_webhook(request: Request, x_telegram_bot_api_secret_token: str = Header(None)) -> dict[str, str]:
...
async def send_message(chat_id: int, text: str, token: str = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"sent": sent, "chat_id": chat_id, "timestamp": _get_now()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/messages/recent")
async def recent_messages(limit: int = 10, token: str = None) -> dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(
            f"{OPENA2_ARCHIVE}/archiv/last?n={limit}", headers={"Content-Type": "application/json"}, method="GET"
        )
        with urllib.request.urlopen(req, timeout=5) as r:
            result = json.loads(r.read().decode())
            ...
            telegram_msgs = [item for item in items if "opena4_telegram" in item.get("path", "")]

            return {"count": len(telegram_msgs), "messages": telegram_msgs[:limit]}
    except Exception as e:
