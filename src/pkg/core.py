...
class AgentManager:
    ...
    async def register_agent(self, agent: Agent) -> None:
        ...
        await self._event_queue.put(EventMessage(event_type="agent_registered", data={"agent": agent.name}))
    ...
    async def update_status(self, agent_name: str, status: str) -> None:
        ...
        if agent_name in self.agents:
            ...
            await self._event_queue.put(
                EventMessage(event_type="agent_status", data={"agent": agent_name, "status": status})
            )
    ...
    async def execute_command(self, agent_name: str, command: str) -> dict:
        ...
        await self._event_queue.put(
            EventMessage(event_type="command_executed", data={"agent": agent_name, "command": command})
        )

        return {"status": "accepted", "agent": agent_name, "command": command, "strict": True}
