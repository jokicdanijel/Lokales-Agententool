...
app = FastAPI(title="opena15_Shop", version="1.0.0", description="E-Commerce / Shop Agent - Product & Order Management")
...
def _apply_discount(total: float, discount_code: str | None) -> dict[str, float]:
    """Apply discount code"""
    discount_rates = {"SAVE10": 0.10, "SAVE20": 0.20, "WELCOME": 0.15}
...
async def list_products(authorization: str = Header(None)):
    ...
    try:
        ...
        for product_id, product in _products.items():
            ...
            products_list.append({"id": product_id, **product, "stock": stock})
...
async def create_order(req: OrderCreateRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        for item in req.items:
            ...
            order_items.append(
                {"product_id": product_id, "quantity": quantity, "price": product["price"], "subtotal": item_total}
            )
...
async def calculate_pricing(req: PricingCalculateRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        logger.info(
            f"💰 Pricing calculated: ${pricing['grand_total']:.2f} (discount: {discount_info['discount_rate']*100:.0f}%)"
        )
        ...
        return {"strict": True, "pricing": pricing, "ts": datetime.utcnow().isoformat() + "Z"}
    except HTTPException:
        raise
    except Exception as e:
    ...
    try:
        if req.action == "add":
            ...
            if req.cart_id not in _carts:
                _carts[req.cart_id] = {"items": {}, "created_at": datetime.utcnow().isoformat()}
...
if __name__ == "__main__":
    ...
    _products[prod2_id] = {"name": "Mouse", "description": "Wireless mouse", "price": 29.99, "category": "accessories"}
