...
app = FastAPI(
    title="opena12_Influencer", version="1.0.0", description="Influencer Management Agent - Campaign & Performance"
)
...
async def list_influencers(authorization: str = Header(None)):
    ...
    try:
        influencers_list = [{**v, "id": k} for k, v in _influencers.items()]
...
async def track_performance(req: PerformanceQueryRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        return {"strict": True, "performance": performance, "ts": datetime.utcnow().isoformat() + "Z"}
...
async def calculate_roi(req: ROICalculationRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        await _archive(
            {"op": "ROI_CALCULATE", "campaign_id": req.campaign_id, "roi_percentage": roi, "revenue": revenue}
        )

        return {"strict": True, "roi": roi_data, "ts": datetime.utcnow().isoformat() + "Z"}
    except HTTPException:
        raise
    except Exception as e:
...
async def analyze_audience(req: PerformanceQueryRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        audience_data = {
            ...
            "demographics": {"age_18_24": 0.25, "age_25_34": 0.40, "age_35_44": 0.20, "age_45_plus": 0.15},
            "interests": ["Technology", "Lifestyle", "Fashion", "Business"],
            "geographic": {"US": 0.45, "EU": 0.35, "APAC": 0.20},
        }

        logger.info(f"👥 Audience analyzed: {req.campaign_id}")

        return {"strict": True, "audience": audience_data, "ts": datetime.utcnow().isoformat() + "Z"}
    except HTTPException:
        raise
    except Exception as e:
