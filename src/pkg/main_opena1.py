...
- Savepoint-Format (bindend):
  ...
  Ablage:  1.opena1&2_portier/archivp/YYYY/MM/DD/<NAME>.json
  Index:   1.opena1&2_portier/archivp/index.jsonl (append-only)
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/opena2_runtime.log"), logging.StreamHandler()],
)
...
# -------------------------------------------------------------------
@app.middleware("http")
async def validate_port_policy(request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
    ...
    if port is not None:
        if port in FORBIDDEN_PORTS:
            raise HTTPException(status_code=403, detail=f"Port {port} ist verboten (OpenWebUI UI-only)")
...
async def health() -> HealthResponse:
    ...
    return HealthResponse(service="opena2", status="healthy", strict=True, timestamp=datetime.now(UTC).isoformat())


@app.post("/store/archivp", response_model=SafepointResponse)
@rate_limiter.limit()
async def store_archivp(
    body: SafepointRequest, token: HTTPAuthorizationCredentials = Security(security)
) -> SafepointResponse:
    ...
    return SafepointResponse(strict=True, written=True, sp_name=sp_name, path=str(out_file))
...
@rate_limiter.limit()
async def finalize(info: dict[str, Any], token: HTTPAuthorizationCredentials = Security(security)) -> dict[str, Any]:
