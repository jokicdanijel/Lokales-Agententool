...
# ========== HELPERS ==========
def test_request(
    method: str, endpoint: str, data: dict[str, Any] = None, params: dict[str, Any] = None, expect_auth: bool = True
) -> requests.Response:
    """Make HTTP request"""
...
def print_test(test_num: int, test_name: str, passed: bool, details: str = ""):
    ...
    print(
        f"{color}[{status}]{reset} Test {test_num} ({test_name}): {'PASS' if passed else 'FAIL'}{' - ' + details if details else ''}"
    )
...
def test_04_get_crypto_prices():
    ...
    try:
        ...
        return print_test(
            4, "Get Crypto Prices", True, f"BTC: ${prices.get('bitcoin')}, ETH: ${prices.get('ethereum')}"
        )
...
def test_05_add_portfolio_stock():
    ...
    try:
        payload = {"symbol": "AAPL", "market": "stock", "quantity": 10.0, "avg_price": 150.0}
        ...
        return print_test(
            ...
            "Add Portfolio Stock",
            True,
            f"Symbol: {position['symbol']}, Qty: {position['quantity']}, Avg: ${position['avg_price']}",
        )
...
def test_06_add_portfolio_crypto():
    ...
    try:
        payload = {"symbol": "bitcoin", "market": "crypto", "quantity": 0.5, "avg_price": 40000.0}
        ...
        if not data.get("success"):
            return print_test(6, "Add Portfolio Crypto", False, "Success=False")
...
def test_07_get_portfolio():
    ...
    try:
        ...
        return print_test(
            7, "Get Portfolio", True, f"Positions: {len(positions)}, Value: ${total_value:.2f}, PnL: ${total_pnl:.2f}"
        )
...
def test_08_create_alert_stock():
    ...
    try:
        ...
        if not data.get("success"):
            return print_test(8, "Create Alert Stock", False, "Success=False")
...
def test_11_delete_alert():
    ...
    try:
        ...
        if not data.get("success"):
            return print_test(11, "Delete Alert", False, "Success=False")
...
def test_12_command_get_prices():
    ...
    try:
        payload = {"action": "get_prices", "params": {"symbols": ["AAPL", "MSFT"], "market": "stock"}}
...
def test_13_command_add_position():
    ...
    try:
        payload = {
            "action": "add_position",
            "params": {"symbol": "TSLA", "market": "stock", "quantity": 5.0, "avg_price": 250.0},
        }
        ...
        if not data.get("success"):
            return print_test(13, "Command add_position", False, "Success=False")

        result = data.get("result", {})
        position = result.get("position", {})

        return print_test(
            13, "Command add_position", True, f"Symbol: {position.get('symbol')}, Qty: {position.get('quantity')}"
        )
...
def test_14_command_get_portfolio():
    ...
    try:
        payload = {"action": "get_portfolio", "params": {}}
        ...
        if not data.get("success"):
            return print_test(14, "Command get_portfolio", False, "Success=False")
