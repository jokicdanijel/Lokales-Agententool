...
class SafepointClient:
    """Safepoint-Client 3.0 für opena19 (Aktien & Crypto)."""
    ...
    async def write(category: str, source: str, destination: str, request_id: str, payload: dict):
        ...
        def mask(o):
            if isinstance(o, dict):
                return {
                    k: ("***" if k.lower() in ["token", "auth", "password", "secret", "apikey", "key"] else mask(v))
                    for k, v in o.items()
                }
            if isinstance(o, list):
                return [mask(i) for i in o]
            return o
    ...
    def write_safepoint(self, destination: str, category: str, request_id: str, payload: Dict[str, Any]) -> str:
...
def create_safepoint_client(agent_id: str) -> SafepointClient:
    ...
    return SafepointClient(agent_id=agent_id, archivp_root="/tmp/archivp_store", dashboard_url="http://127.0.0.1:12349")
...
if __name__ == "__main__":
    ...
    # CMD Safepoint
    cmd_file = client.write_cmd_safepoint(
        "demo_req_001",
        {"command": "telegram_send", "message": "Hello Telegram", "token": "bot-secret-key"},  # Wird maskiert
    )
    print(f"✓ CMD Safepoint: {cmd_file}")

    # RESP Safepoint
    resp_file = client.write_resp_safepoint(
        "demo_req_001", {"success": True, "response": "Message sent successfully", "message_id": 12345}
    )
