...
async def chat_with_openwebui(request: ChatRequest, token: HTTPAuthorizationCredentials = Security(security)):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(f"{OPENWEBUI_URL}/api/chat/completions", json=openwebui_payload, timeout=30.0)
...
async def fallback_chat_response(request: ChatRequest) -> dict[str, Any]:
    ...
    return ChatResponse(
        ...
        usage={
            ...
            "total_tokens": len(request.prompt.split()) + len(fallback_message.split()),
        },
        timestamp=conversation_entry["timestamp"],
    ).model_dump()
...
async def publish_chat_event(conversation: dict[str, Any]) -> None:
    ...
    try:
        async with httpx.AsyncClient() as client:
            event_data = {"event_type": "opena3_chat", "agent_id": AGENT_ID, "data": conversation}
...
async def get_conversations(
    token: HTTPAuthorizationCredentials = Security(security), limit: int = 50
) -> list[dict[str, Any]]:
...
async def clear_conversations(token: HTTPAuthorizationCredentials = Security(security)) -> dict[str, Any]:
    ...
    return {"message": f"Cleared {cleared_count} conversations", "timestamp": datetime.now(UTC).isoformat()}


# Session Management
@app.get("/sessions")
async def get_active_sessions(token: HTTPAuthorizationCredentials = Security(security)) -> dict[str, Any]:
...
async def openwebui_status(token: HTTPAuthorizationCredentials = Security(security)) -> dict[str, Any]:
...
# Command Interface
@app.post("/command")
async def execute_command(payload: dict[str, Any], token: HTTPAuthorizationCredentials = Security(security)) -> Any:
