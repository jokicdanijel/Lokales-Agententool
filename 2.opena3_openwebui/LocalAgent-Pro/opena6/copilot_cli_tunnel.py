...
PROMPT_COMMIT_DEFAULT = (
    "Erzeuge eine prägnante, konventionelle Commit-Message für den folgenden Git-Diff (Deutsch)."
)

PROMPT_COMMIT_EN = (
    "Generate a concise, conventional commit message for the following Git diff (English)."
)
...
class CopilotResponse(BaseModel):
    ...
    mode: Optional[str] = Field(None, description="Modus (chat|explain|commit)")
    preview: bool = Field(default=False, description="Nur Preview")
    base: Optional[str] = Field(None, description="Base-Verzeichnis für Logs")
    command: Optional[List[str]] = Field(None, description="Ausgeführter Befehl")
    cwd: Optional[str] = Field(None, description="Arbeitsverzeichnis")
    ...
    log_path: Optional[str] = Field(None, description="Pfad zur Log-Datei")
...
class Tools:
    ...
    class Valves(BaseModel):
        ...
        MODE: str = Field(
            default="chat",
            description="Modus: chat | explain | commit"
        )
        PROMPT: str = Field(
            default="",
            description="Prompt-Text für MODE=chat/commit"
        )
        FILES: List[str] = Field(
            default_factory=list,
            description="Relevante Dateien (explain nutzt FILES[0])"
        )
        CWD: str = Field(
            default=".",
            description="Arbeitsverzeichnis/Git-Repository"
        )
        OUT_DIR: str = Field(
            default="/mnt/data",
            description="Zielordner für Logs (Fallback: /tmp)"
        )
        PREVIEW_ONLY: bool = Field(
            default=False,
            description="Nur anzeigen, nicht ausführen"
        )
        TIMEOUT_SEC: int = Field(
            default=120,
            description="Timeout für gh-Aufruf in Sekunden"
        )
        TRUNCATE: int = Field(
            default=8000,
            description="Stdout/Stderr auf N Zeichen kürzen (0=kein Kürzen)"
        )
        LANGUAGE: str = Field(
            default="de",
        ...
        INCLUDE_TIMESTAMPS: bool = Field(
            default=True,
            description="Zeitstempel in Logs speichern"
        )
        VERBOSE: bool = Field(
            default=False,
        ...
        try:
            ...
            if not gh_path:
                return await self._error_response(
                    ...
                    "Bitte installieren & 'gh auth login' ausführen.",
                    __event_emitter__
                )
            ...
            if not cmd:
                return await self._error_response(
                    f"Befehl konnte nicht erstellt werden",
                    __event_emitter__
                )
            ...
            await self._notify(
                __event_emitter__,
                f"✅ Copilot {mode} abgeschlossen" if result.get("ok") else f"❌ Fehler in {mode}"
            )
    ...
    async def _validate_cwd(self, cwd_str: str) -> Optional[Path]:
    ...
    async def _build_command(
        ...
        if mode == "chat":
            ...
            prompt = v.PROMPT.strip() or PROMPTS.get(
                f"commit_{v.LANGUAGE}", PROMPT_COMMIT_DEFAULT
            )
            ...
            await self._notify(
                emitter,
                f"❌ Unbekannter MODE: {mode} (erlaubt: chat|explain|commit)"
            )
            return None

    async def _capture_git_diff(self, cwd: Path) -> Optional[str]:
