...
class DispatcherClient:
    ...
    def __init__(self, agent_name: str = '5.opena6_browser', bearer_token: str = None,
    ...
    def register_agent(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f'{self.dispatcher_url}/register',
                json=payload,
            ...
            if response.status_code == 200:
                logger.info(f"✅ Agent registered with dispatcher")
                return True
            else:
    ...
    def fetch_command(self) -> Optional[Dict[str, Any]]:
        """Fetch pending command from archivator"""
        try:
            response = requests.get(
                f'{self.archivator_url}/pending',
                headers=self.headers,
                timeout=5
            )
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('command'):
                    logger.info(f"📥 Command fetched from archivator")
    ...
    def report_result(self, result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f'{self.archivator_url}/safepoint',
                json=payload,
                headers=self.headers,
                timeout=5
            )
            ...
            if response.status_code in [200, 201]:
                logger.info(f"✅ Result reported to archivator")
                return True
            else:
    ...
    def get_agent_status(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f'{self.dispatcher_url}/status/{self.agent_name}',
                headers=self.headers,
                timeout=5
            )
...
class SafepointManager:
    ...
    def __init__(self, agent_name: str = '5.opena6_browser'):
        ...
        self.agent_name = agent_name
        self.safepoints = {}
        logger.info(f"✅ SafepointManager initialized")
    ...
    def get_safepoint(self, safepoint_id: str) -> Optional[Dict[str, Any]]:
