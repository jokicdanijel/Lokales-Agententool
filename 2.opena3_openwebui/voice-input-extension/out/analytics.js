...
exports.globalEventLogger = exports.globalAnalytics = exports.EventLogger = exports.AnalyticsManager = void 0;
/**
...
class AnalyticsManager {
    ...
    getMetrics() {
        ...
        const mostUsedLanguage = Object.entries(languageCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || 'unknown';
    ...
    getSessionsInRange(startDate, endDate) {
        return this.sessions.filter(s => s.startTime >= startDate && s.startTime <= endDate);
    }
    /**
...
class EventLogger {
     ...
     */
    exportLogsAsText() {
        return this.logs.map(log => `[${log.timestamp.toISOString()}] ${log.level.toUpperCase()} - ${log.message}${log.details ? '\nDetails: ' + JSON.stringify(log.details, null, 2) : ''}`).join('\n\n');
    }
    /**
     * Clear all logs
     */
