...
def setup_logging():
    ...
    logging.basicConfig(
        level=log_config.get_numeric_level(),
        handlers=handlers,
        force=True
    )
    ...
    for attempt in range(retry_config.max_retries + 1):
        try:
            ...
            if retry_config.retry_on_connection_error and attempt < retry_config.max_retries:
                delay = retry_config.get_delay(attempt)
                logger.warning(f"🔄 Retry {attempt + 1}/{retry_config.max_retries} nach Connection-Error ({delay:.1f}s)")
                time.sleep(delay)
            else:
...
class ChatRequest(BaseModel):
    ...
    model: Optional[str] = Field(default=None, description="Modell-Alias (z.B. 'llama3.1', 'gpt-4')")
...
class StreamChatRequest(BaseModel):
    ...
    message: str = Field(..., description="Chat-Nachricht")
    model: Optional[str] = Field(default=None, description="Modell-Alias")
    context: Optional[Dict[str, Any]] = Field(default=None, description="Optionaler Kontext")
...
def mask_secrets(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: "***MASKED***" if isinstance(k, str) and k.lower() in ["token", "password", "secret", "key", "bearer"] else mask_secrets(v)
...
async def list_models(
    request: Request,
    include_backend: bool = Query(default=True, description="Backend-Modelle einbeziehen")
) -> ModelListResponse:
    """
    ...
    if default_model_obj is not None:
        available_aliases: List[str] = list(model_registry.available_aliases) if hasattr(model_registry, 'available_aliases') else []
...
async def generate_sse_stream(
    ...
    write_resp_safepoint: bool = True
    ...
    try:
        ...
        if write_resp_safepoint:
            write_safepoint("opena3", "dashboard", "RESP", {
                "model": model_id,
                "status": "error",
    ...
    try:
        ...
        error = {
            "error": {
                "code": "CHAT_FAILED",
                "message": str(e),
                "details": {"message": body.message[:100]}
            }
        }
