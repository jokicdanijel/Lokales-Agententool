...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("logs/opena3_v2.log", mode="a", encoding="utf-8")
    ]
)
...
class OpenWebUIClient:
    ...
    async def chat(self, prompt: str, model: Optional[str] = None, context: Optional[Dict] = None) -> Dict[str, Any]:
        """Chat mit OpenWebUI"""
        if config.MOCK_MODE:
            return {
...
async def handle_cmd(cmd: CMDEnvelope, token: str = Depends(verify_token)) -> RESPEnvelope:
    ...
    # CMD Safepoint
    cmd_sp_id = safepoint_manager.write_safepoint(
        agent=config.SERVICE_ID,
        category="CMD",
        body=cmd.model_dump()
    )
    ...
    try:
        ...
        if cmd.command == "chat":
            ...
            # Dashboard Event
            await dashboard_client.publish_event("opena3_chat", {
    ...
    # RESP Safepoint
    safepoint_manager.write_safepoint(
        agent=config.SERVICE_ID,
        category="RESP",
