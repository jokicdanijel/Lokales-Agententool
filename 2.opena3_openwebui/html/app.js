...
class OpenWebUIDashboard {
  ...
  renderCapabilities() {
    ...
    container.innerHTML = this.config.capabilities
      .map(
        (cap) => `
  ...
  renderModelSelector() {
    ...
    selector.innerHTML = this.config.models
      .map(
        (m) =>
  ...
  async loadModels() {
    ...
    try {
      const data = await this.apiCall("/api/models").catch(() => ({
  ...
  async loadHistory() {
    ...
    try {
      const data = await this.apiCall("/api/chat/history").catch(() => ({
        conversations: [],
      }));
      ...
      if (conversations.length === 0) {
        container.innerHTML =
          '<p class="text-muted">No conversation history</p>';
        return;
      }
  ...
  async loadTools() {
    ...
    try {
      const data = await this.apiCall("/api/tools").catch(() => ({
        tools: [],
      }));
  ...
  async loadFiles() {
    ...
    try {
      const data = await this.apiCall("/api/files").catch(() => ({
        files: [],
      }));
  ...
  async uploadFile() {
    ...
    try {
      const response = await fetch(
        `${this.config.api.baseUrl}/api/files/upload`,
        {
          method: "POST",
  ...
  initSSE() {
    ...
    container.innerHTML =
      '<div class="stream-event">Connecting to SSE...</div>';

    try {
      const token = this.getToken();
      this.sseConnection = new EventSource(
        `${this.config.api.baseUrl}/sse/events?token=${token}`,
      );
  ...
  async loadLogs() {
    ...
    try {
      ...
      if (entries.length === 0) {
        container.innerHTML =
          '<div class="log-entry info"><span class="log-time">--:--:--</span> No logs available</div>';
        return;
      }
  ...
  getToken() {
    const input = document.getElementById("bearerToken");
    return (
      input?.value || localStorage.getItem(this.config.auth.tokenKey) || ""
    );
  }
