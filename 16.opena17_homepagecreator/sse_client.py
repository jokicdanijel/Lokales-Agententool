...
class SSEClient:
    ...
    async def publish_event(self, event_type: str, data: dict[str, Any], category: str = "agent") -> bool:
        """
    ...
    async def publish_site_generated(self, site_id: str, site_name: str, pages_count: int, generator: str) -> bool:
        """Publiziert Site-Generated Event"""
        return await self.publish_event(
            event_type="site_generated",
            data={"site_id": site_id, "site_name": site_name, "pages_count": pages_count, "generator": generator},
            category="homepage_creator",
        )

    async def publish_site_exported(self, site_id: str, format: str, file_size_bytes: int) -> bool:
        """Publiziert Site-Exported Event"""
        return await self.publish_event(
            event_type="site_exported",
            data={"site_id": site_id, "format": format, "file_size_bytes": file_size_bytes},
            category="homepage_creator",
        )

    async def publish_site_deployed(self, site_id: str, target: str, deployment_url: str | None) -> bool:
        """Publiziert Site-Deployed Event"""
        return await self.publish_event(
            event_type="site_deployed",
            data={"site_id": site_id, "target": target, "deployment_url": deployment_url},
            category="homepage_creator",
        )

    async def publish_health_status(self, status: str, uptime_seconds: float, total_sites: int) -> bool:
        """Publiziert Health-Status Event"""
        return await self.publish_event(
            event_type="health_status",
            data={"status": status, "uptime_seconds": uptime_seconds, "total_sites": total_sites},
            category="health",
        )

    async def publish_error(self, error_code: str, error_message: str, details: dict[str, Any] | None = None) -> bool:
        """Publiziert Error Event"""
        return await self.publish_event(
            event_type="error",
            data={"code": error_code, "message": error_message, "details": details or {}},
            category="error",
        )
