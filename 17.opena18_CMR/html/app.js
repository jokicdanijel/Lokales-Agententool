...
class CRMDashboard {
    ...
    async loadMetrics() {
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById('revenue').textContent = '€' + (data.pipeline_value || 0);
    ...
    async loadRecentActivity() {
        ...
        try {
            ...
            container.innerHTML = '<div class="loading">Aktivitäten werden geladen...</div>';
        }
    }
    ...
    showAddLeadModal() { this.showToast('Lead-Formular wird geöffnet', 'info'); }
    showAddDealModal() { this.showToast('Deal-Formular wird geöffnet', 'info'); }
    showAddTaskModal() { this.showToast('Task-Formular wird geöffnet', 'info'); }
    showAddCompanyModal() { this.showToast('Firmen-Formular wird geöffnet', 'info'); }
    
    exportData() { this.showToast('Export wird vorbereitet...', 'info'); }
    generateReport() { this.showToast('Report wird generiert...', 'info'); }
    useTemplate(template) { this.showToast(`Template: ${template}`, 'info'); }
