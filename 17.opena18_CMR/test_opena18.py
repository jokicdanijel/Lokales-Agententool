...
def test_health() -> None:
    ...
    print(
        f"[✓] Test 1 (Health): PASS - Uptime: {data['uptime_seconds']:.2f}s, Contacts: {data['total_contacts']}, Orgs: {data['total_organizations']}, Deals: {data['total_deals']}"
    )
...
def test_update_contact(contact_id: str) -> None:
    ...
    payload = {"position": "Managing Director", "phone": "+49 987 654321"}
...
def test_delete_contact() -> None:
    ...
    payload = {"first_name": "Delete", "last_name": "Test", "email": "delete.test@example.com"}
...
def test_update_organization(organization_id: str) -> None:
    ...
    payload = {"size": "enterprise", "website": "https://www.acme.example.com"}
...
def test_create_deal(contact_id: str, organization_id: str) -> dict[str, Any]:
    ...
    print(
        f"[✓] Test 9 (Create Deal): PASS - Deal ID: {deal_id}, Value: {data['value']} {data['currency']}, Stage: {data['stage']}"
    )
    return data
...
def test_create_activity(contact_id: str, deal_id: str) -> None:
    ...
    print(
        f"[✓] Test 11 (Create Activity): PASS - Activity ID: {data['activity_id']}, Type: {data['activity_type']}, Duration: {data['duration_minutes']}min"
    )
...
def test_search() -> None:
    ...
    payload = {"query": "Max", "entity_types": ["contacts", "organizations", "deals"], "max_results": 50}
    ...
    print(
        f"[✓] Test 13 (Search): PASS - Contacts: {len(data['contacts'])}, Orgs: {len(data['organizations'])}, Deals: {len(data['deals'])}"
    )
...
def test_command_endpoint() -> None:
    ...
    payload = {
        ...
        "params": {"first_name": "Jane", "last_name": "Doe", "email": "jane.doe@example.com", "position": "CTO"},
    }
    ...
    print(
        f"[✓] Test 14 (Command Endpoint): PASS - Action: {data['action']}, Contact: {data['result']['first_name']} {data['result']['last_name']}"
    )
...
def test_strict_json() -> None:
    ...
    print("[✓] Test 15 (Strict JSON): PASS - Unknown fields rejected (422)")
