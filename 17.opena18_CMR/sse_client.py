...
class SSEClient:
    ...
    async def publish_event(self, event_type: str, data: dict[str, Any], category: str = "crm") -> bool:
    ...
    async def publish_contact_created(self, contact_id: str, email: str) -> bool:
        ...
        return await self.publish_event("contact_created", {"contact_id": contact_id, "email": email})

    async def publish_deal_updated(self, deal_id: str, stage: str, value: float) -> bool:
        """Publiziert Deal-Updated Event"""
        return await self.publish_event("deal_updated", {"deal_id": deal_id, "stage": stage, "value": value})

    async def publish_activity_logged(self, activity_id: str, activity_type: str) -> bool:
        """Publiziert Activity-Logged Event"""
        return await self.publish_event("activity_logged", {"activity_id": activity_id, "type": activity_type})
