...
class DealCreate(BaseModel):
    ...
    organization_id: str | None = Field(default=None)
    close_date: str | None = Field(default=None)  # ISO-Format
...
class CRMManager:
    ...
    def create_contact(req: ContactCreate) -> Contact:
        ...
        DataStore.log_history(
            "create_contact",
            {"contact_id": contact.contact_id, "email": contact.email, "organization_id": contact.organization_id},
        )
    ...
    def update_contact(contact_id: str, req: ContactUpdate) -> Contact:
        ...
        # History Log
        DataStore.log_history("update_contact", {"contact_id": contact_id, "updated_fields": list(update_data.keys())})

        return contact

    @staticmethod
    def list_contacts(
        organization_id: str | None = None, search: str | None = None, max_results: int = 100
    ) -> list[Contact]:
        """Liste Kontakte"""
        contacts = DataStore.load_contacts()
    ...
    def delete_contact(contact_id: str) -> None:
        ...
        # History Log
        DataStore.log_history("delete_contact", {"contact_id": contact_id, "email": contact.email})
    ...
    def update_organization(organization_id: str, req: OrganizationUpdate) -> Organization:
        ...
        # History Log
        DataStore.log_history(
            "update_organization", {"organization_id": organization_id, "updated_fields": list(update_data.keys())}
        )

        return organization

    @staticmethod
    def list_organizations(search: str | None = None, max_results: int = 100) -> list[Organization]:
        ...
        if search:
            ...
            organizations = [
                ...
                if search_lower in o.name.lower() or (o.industry and search_lower in o.industry.lower())
            ]
    ...
    def delete_organization(organization_id: str) -> None:
        ...
        # History Log
        DataStore.log_history("delete_organization", {"organization_id": organization_id, "name": organization.name})
    ...
    def create_deal(req: DealCreate) -> Deal:
        ...
        # History Log
        DataStore.log_history(
            "create_deal", {"deal_id": deal.deal_id, "title": deal.title, "value": deal.value, "stage": deal.stage}
        )
    ...
    def update_deal(deal_id: str, req: DealUpdate) -> Deal:
        ...
        # History Log
        DataStore.log_history(
            "update_deal", {"deal_id": deal_id, "updated_fields": list(update_data.keys()), "new_stage": deal.stage}
        )

        return deal

    @staticmethod
    def list_deals(stage: str | None = None, contact_id: str | None = None, max_results: int = 100) -> list[Deal]:
        """Liste Deals"""
        deals = DataStore.load_deals()
    ...
    def delete_deal(deal_id: str) -> None:
        ...
        # History Log
        DataStore.log_history("delete_deal", {"deal_id": deal_id, "title": deal.title})
    ...
    @staticmethod
    def list_activities(
        contact_id: str | None = None, deal_id: str | None = None, max_results: int = 100
    ) -> list[Activity]:
        """Liste Aktivitäten"""
        activities = DataStore.load_activities()
    ...
    def search(req: SearchRequest) -> dict[str, list[Any]]:
        """Globale Suche"""
        results = {"contacts": [], "organizations": [], "deals": []}
        ...
        if "organizations" in req.entity_types:
            ...
            matches = [
                o.model_dump()
                for o in organizations
                if query_lower in o.name.lower() or (o.industry and query_lower in o.industry.lower())
            ]
        ...
        if "deals" in req.entity_types:
            deals = DataStore.load_deals()
            matches = [d.model_dump() for d in deals if query_lower in d.title.lower()]
...
app = FastAPI(title=f"{SERVICE_NAME} - CRM Agent", version=VERSION, docs_url="/docs", redoc_url="/redoc")
...
async def root():
    ...
    return {
        ...
        "port": PORT,
        "status": "running",
        "endpoints": ["/health", "/contacts", "/organizations", "/deals", "/activities", "/search", "/command"],
    }
...
async def list_organizations(
    search: str | None = None, max_results: int = 100, token: str = Depends(verify_bearer_token)
):
    """Liste Organisationen"""
