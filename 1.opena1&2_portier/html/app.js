...
class PortierDashboard {
  ...
  async init() {
    console.log(
      `🔵 Portier Dashboard v${this.config.agent.version} initializing...`,
    );
    this.bindNavigation();
    this.bindActions();
    this.bindModals();
  ...
  bindActions() {
    document
      ...
      ?.addEventListener("click", () => this.checkAllHealth());
    document
      .getElementById("dispatchBtn")
      ?.addEventListener("click", () => this.showDispatchModal());
    document
      .getElementById("createSafepointBtn")
      ?.addEventListener("click", () => this.showCreateSafepointModal());
    document
  ...
  bindModals() {
    ...
    document
      .getElementById("sendDispatchBtn")
      ?.addEventListener("click", () => this.sendDispatch());
    document
      .getElementById("createSafepointSubmitBtn")
      ?.addEventListener("click", () => this.createSafepoint());
  }
  ...
  async loadOverview() {
    ...
    this.updateMetric(
      "agentsActive",
      status1.active_agents || this.config.registeredAgents.length,
    );
  ...
  async loadCoordinatorData() {
    ...
    if (!status.error) {
      document.getElementById("kordpUptime")?.textContent =
        status.uptime || "N/A";
      document.getElementById("kordpVersion")?.textContent =
        status.version || "1.0.0";
      document.getElementById("kordpRequests")?.textContent =
  ...
  async loadArchivatorData() {
    ...
    if (!status.error) {
      document.getElementById("archivpUptime")?.textContent =
        status.uptime || "N/A";
      document.getElementById("archivpVersion")?.textContent =
        status.version || "1.0.0";
    }

    if (!stats.error) {
      document.getElementById("totalSafepoints")?.textContent =
        stats.total || 0;
  ...
  async loadSafepoints() {
    const result = await this.opena2Call(
      this.config.api.opena2.safepoints + "?limit=50",
    );
  ...
  renderSafepoints() {
    ...
    container.innerHTML = this.state.safepoints
      .map(
        (sp) => `
  ...
  async viewSafepoint(id) {
    const result = await this.opena2Call(
      `${this.config.api.opena2.safepoints}/${id}`,
    );
    if (!result.error) {
      document.getElementById("safepointDetailContent").textContent =
  ...
  async pingAgent(agentId, port) {
    ...
    try {
      ...
      if (data.status === "ok") {
        ...
        this.showToast(
          `${agentId} antwortet, aber Status: ${data.status}`,
          "warning",
        );
      }
    } catch (e) {
  ...
  async sendDispatch() {
    ...
    const result = await this.opena1Call(
      this.config.api.opena1.dispatch,
      "POST",
      {
        tool,
        payload: payloadJson,
    ...
    if (!result.error) {
      document.getElementById("dispatchResult").textContent = JSON.stringify(
        result,
        null,
        2,
      );
  ...
  async loadLogs() {
    ...
    const logs = [
      ...
      },
      {
        time: new Date().toISOString(),
        level: "INFO",
        message: "opena1 health check passed",
      },
      {
        time: new Date().toISOString(),
        level: "INFO",
        message: "opena2 health check passed",
      },
    ];
  ...
  async testApi() {
    ...
    const baseUrl =
      agent === "opena1"
        ? this.config.opena1.baseUrl
        : this.config.opena2.baseUrl;
