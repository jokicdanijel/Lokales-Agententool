...
SAFEPOINTS = {
    "gateway": {"name": "Gateway Checkpoint", "status": "✅ Active", "rate": 0, "last_checkpoint": datetime.now().isoformat()},
    "tool_exec": {"name": "Tool Execution", "status": "✅ Active", "rate": 0, "last_checkpoint": datetime.now().isoformat()},
    "archive": {"name": "Archive Access", "status": "✅ Active", "rate": 0, "last_checkpoint": datetime.now().isoformat()}
}
...
async def check_agent_health(agent_name: str, port: int, description: str) -> dict:
    ...
    try:
        ...
        add_log(f"⚠️ Agent {agent_name} nicht erreichbar: {str(e)}", "warning")
...
async def run_e2e():
    ...
    try:
        ...
        message = f"✅ E2E Test PASSED ({duration_ms:.0f}ms)" if all_passed else f"❌ E2E Test FAILED ({duration_ms:.0f}ms)"
        ...
        add_log(f"❌ E2E Test Error: {str(e)}", "error")
        return {
            "success": False,
            "error": str(e),
            "timestamp": datetime.now().isoformat()
...
async def get_circuit_logs():
    ...
    return {
        "logs": circuit_logs[-30:],
        "total": len(circuit_logs),
        "timestamp": datetime.now().isoformat()
    }
...
async def general_exception_handler(request, exc):
    add_log(f"Unerwarteter Fehler: {str(exc)}", "error")
    return JSONResponse(
        status_code=500,
        content={"detail": "Interner Fehler", "error": str(exc)}
    )
