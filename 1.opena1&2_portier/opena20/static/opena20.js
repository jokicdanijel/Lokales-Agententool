...
const opena20 = (function () {
  ...
  function updateDashboard(data) {
    ...
    if (data.safepoints) {
      ...
      if (sps.gateway) {
        document.getElementById("gw_status").textContent =
          sps.gateway.status || "-";
        document.getElementById("gw_last").textContent =
          sps.gateway.last_checkpoint || "-";
        document.getElementById("gw_time").textContent =
          sps.gateway.timestamp || "-";
      }

      if (sps.tool_exec) {
        document.getElementById("tool_status").textContent =
          sps.tool_exec.status || "-";
        document.getElementById("tool_last").textContent =
          sps.tool_exec.last_checkpoint || "-";
        document.getElementById("tool_time").textContent =
          sps.tool_exec.timestamp || "-";
      }

      if (sps.archive) {
        document.getElementById("arch_status").textContent =
          sps.archive.status || "-";
        document.getElementById("arch_last").textContent =
          sps.archive.last_checkpoint || "-";
        document.getElementById("arch_time").textContent =
          sps.archive.timestamp || "-";
      }
    }
  }
  ...
  const SystemControl = {
    async restart() {
      if (
        !window.confirm(
          "🚨 Wirklich den System neu starten? Dies unterbricht aktive Operationen!",
        )
      )
        return;
  ...
  const UI = {
    ...
    setupButtons() {
      ...
      const restartBtn = document.querySelector(
        '[onclick="window.restartSystem()"]',
      );

      if (e2eBtn)
        e2eBtn.addEventListener("click", (e) => {
          e.preventDefault();
          E2ETester.run();
        });
