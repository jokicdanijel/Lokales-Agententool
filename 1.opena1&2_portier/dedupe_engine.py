...
class DedupeEngine:
    ...
    def _load_heads(self) -> None:
        ...
        if self.heads_file.exists():
            try:
                with open(self.heads_file) as f:
    ...
    def register_safepoint(self, sp_path: str, content: dict, source_agent: str) -> dict[str, Any]:
        """
        ...
        return {"is_duplicate": False, "hash": sha256, "record": asdict(record), "occurrence_count": 1}

    def append_head(self, new_hash: str, meta: dict | None = None) -> None:
        """
        ...
        """
        now_utc = datetime.now(UTC).isoformat().replace("+00:00", "Z")
        entry = {"hash": new_hash, "sequence": len(self.heads), "timestamp": now_utc, "meta": meta or {}}
    ...
    def create_checkpoint(self, window_label: str = "default") -> dict | None:
        """
        ...
        now_utc = datetime.now(UTC).isoformat().replace("+00:00", "Z")
        checkpoint = {
            "checkpoint_id": f"CP_{int(datetime.now(UTC).timestamp())}",
    ...
    def get_dedupe_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "savings_factor": (
                f"{(total_occurrences - duplicate_occurrences) / max(1, duplicate_occurrences):.2f}x"
...
class SafepointManager:
    ...
    def __init__(self, archiv_root: Path = Path("archivp"), dedupe_engine: DedupeEngine | None = None):
        """
    ...
    def write_safepoint(self, sp_path: str, content: dict, source_agent: str) -> dict[str, Any]:
        """
    ...
    def read_safepoint(self, sp_path: str) -> dict | None:
        """
...
if __name__ == "__main__":
    ...
    # Write
    result = manager.write_safepoint("2025/11/10/SP1731155200_opena1->opena2_CMD.json", sp, "opena1")
