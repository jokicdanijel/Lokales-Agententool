...
from paths_config import ARCHIVP_INDEX, ARCHIVP_STORE, BASE_ROOT
...
def _write_safepoint(item: StoreIn) -> Path:
    ...
    with INDEX_FILE.open("a", encoding="utf-8") as idx:
        idx.write(
            json.dumps(
                {"sp": name, "ts": item.ts, "src": item.src, "dst": item.dst, "kind": item.kind, "path": str(fpath)}
            )
            + "\n"
        )
    return fpath
...
async def finalize(body: dict[str, Any]) -> dict[str, Any]:
    ...
    # Store CMD safepoint
    sp = StoreIn(src=body.get("source", "opena1"), dst="archivp", kind="CMD", body=body, strict=True)
    path = _write_safepoint(sp)

    # Return minimal OK response
    return {"ok": True, "request_id": req_id, "stored": str(path), "strict": True}
...
async def store_resp(body: dict[str, Any]) -> dict[str, Any]:
    ...
    sp = StoreIn(src=body.get("source", "tool"), dst="opena1", kind="RESP", body=body, strict=True)
    path = _write_safepoint(sp)
