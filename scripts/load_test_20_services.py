...
async def dispatch_request(
    client: httpx.AsyncClient, service: dict[str, Any], request_id: int, semaphore: asyncio.Semaphore, metrics: Metrics
):
    ...
    async with semaphore:
        ...
        try:
            ...
            response = await client.post(
                f"{PORTIER_URL}/dispatch/kordp", json=payload, headers=headers, timeout=REQUEST_TIMEOUT
            )
...
async def run_load_test():
    ...
    if ARCHIV_INDEX.exists():
        ...
        new_entries = [e for e in entries if e.get("timestamp", "") >= test_start_iso]
    ...
    with open(output_file, "w") as f:
        json.dump(
            {
                "test_timestamp": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
                "config": {
