...
def eval_policy(root: str) -> EvalCategory:
    ...
    cat.add_check(
        ".env not sourced in scripts", len(bad_scripts) == 0, f"Found: {bad_scripts[0]}" if bad_scripts else ""
    )
...
def eval_infra(root: str) -> EvalCategory:
    ...
    if os.path.exists(runbook):
        with open(runbook) as f:
            html = f.read()
        cat.add_check("HTML Runbook valid", "<!doctype html>" in html.lower() and "</html>" in html.lower())
    else:
...
def eval_config(root: str) -> EvalCategory:
    ...
    if os.path.exists(env_file):
        ...
        cat.add_check("No placeholders", not re.search(r"(YOUR_|CHANGE_ME|FIXME|TODO)", env, re.IGNORECASE))
    else:
        cat.add_check(".env exists", False)
...
def eval_quality(root: str) -> EvalCategory:
    ...
    if os.path.exists(gi_file):
        with open(gi_file) as f:
            gi = f.read()
        cat.add_check(".gitignore covers .env, venv, cache", all(x in gi for x in [".env", ".venv", "__pycache__"]))
    else:
        cat.add_check(".gitignore exists", False)

    cat.add_check("README exists", any(os.path.exists(os.path.join(root, f)) for f in ["README.md", "README.rst"]))
