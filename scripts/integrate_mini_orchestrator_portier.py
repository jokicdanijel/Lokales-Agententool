...
def register_at_portier() -> bool:
    ...
    try:
        with httpx.Client(timeout=10.0) as client:
            r = client.post(f"{PORTIER_URL}/route/update", json=payload, headers=headers)
...
def test_dispatch() -> bool:
    ...
    try:
        ...
        with httpx.Client(timeout=30.0) as client:
            r = client.post(f"{PORTIER_URL}/dispatch/kordp", json=payload, headers=headers)
...
def verify_archiv() -> None:
    ...
    if miniorchp_entries:
        ...
        print("      (möglicherweise noch nicht persistiert)")
...
def print_summary() -> None:
    ...
    print('       "action": "send_mail",')
    print('       "params": {"to": "user@example.com", "subject": "Test"}')
    print("     }'")
