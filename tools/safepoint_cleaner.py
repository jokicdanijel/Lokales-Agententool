                ...
                for day_dir in month_dir.glob("??"):
                    ...
                    try:
                        ...
                        if dir_date < cutoff_date:
                            ...
                            shutil.move(str(day_dir), str(archive_target / f"{year_dir.name}_{month_dir.name}_{day_dir.name}"))
                            archived_count += 1
        ...
        for json_file in self.archivp_root.rglob("SP*.json"):
            ...
            try:
                ...
                if "→" not in json_file.name:
                    ...
                    validation_results["validation_errors"].append({
                        "file": str(json_file),
                        "error": "Missing Unicode arrow in filename"
                    })
                    continue
        ...
        if self.stats["errors"]:
            report_lines.extend([
                ...
                *[f"  • {error}" for error in self.stats["errors"][:10]],
                "" if len(self.stats["errors"]) <= 10 else f"  • ... and {len(self.stats['errors']) - 10} more errors",
                ""
            ])
