        ...
        if check_gh_auth() and not dry_run:
            ...
            result = run(
                [
                    ...
                    "--fill",
                    "--body",
                    f"Automated PR by Portier agents\n\nBranch: {branch}\nMessage: {message}",
                    "--title",
                    message,
                ],
                check=False,
...
def main():
    ...
    parser.add_argument("message", nargs="?", default="AGENT: automated changes", help="Commit-Nachricht")
    parser.add_argument("--dry-run", action="store_true", help="Nur simulieren, keine echten Änderungen")
