...
def check_importable(dist, attr, value):
    try:
        ...
        raise DistutilsSetupError(f"{attr!r} must be importable 'module:attrs' string (got {value!r})") from e
...
def check_package_data(dist, attr, value):
    ...
    if not isinstance(value, dict):
        raise DistutilsSetupError(
            f"{attr!r} must be a dictionary mapping package names to lists of " "string wildcard patterns"
        )
...
class Distribution(_Distribution):
    ...
    def _normalize_requires(self):
        ...
        self.extras_require = dict_((k, list(map(str, _reqs.parse(v or [])))) for k, v in extras_require.items())
    ...
    def _find_pattern(pattern: str, enforce_match: bool = True) -> list[str]:
        ...
        if pattern.startswith((os.sep, "/")) or ":\\" in pattern:
            raise InvalidConfigError(f"Pattern {pattern!r} should be relative and must not start with '/'")
    ...
    def _is_setuptools_section(self, section: str) -> bool:
        return section == "metadata" or section.startswith("options") or section in _setuptools_commands()
