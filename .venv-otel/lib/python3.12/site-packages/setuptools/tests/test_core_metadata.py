...
class TestParityWithMetadataFromPyPaWheel:
    ...
    def dist(self, request, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Distribution, "_finalize_license_files", Mock(return_value=None))
...
def _normalize_metadata(msg: Message) -> str:
    ...
    reqs = [_normalize_req(req, extras) for req in _reqs.parse(msg.get_all("Requires-Dist", []))]
