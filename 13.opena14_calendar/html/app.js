class CalendarDashboard {
  ...
  bindEvents() {
    document
      ...
      ?.addEventListener("submit", (e) => this.createEvent(e));
  }
  setTodayDate() {
    ...
    document.getElementById("date-today").textContent =
      today.toLocaleDateString(CONFIG.UI.DATE_LOCALE);
  }
  ...
  async createEvent(e) {
    ...
    const res = await this.apiCall(
      CONFIG.ENDPOINTS.EVENTS_CREATE,
      "POST",
      event,
    );
    if (res?.event_id) {
  ...
  renderTodayEvents() {
    ...
    if (!todayEvents.length) {
      container.innerHTML = '<p class="empty-state">Keine Events heute</p>';
      return;
    }
  ...
  renderUpcomingEvents() {
    ...
    if (!upcoming.length) {
      container.innerHTML = '<p class="empty-state">Keine kommenden Events</p>';
      return;
    }
  ...
  renderCalendar() {
    ...
    document.getElementById("calendar-month").textContent =
      `${CONFIG.MONTHS[this.currentMonth]} ${this.currentYear}`;
    let html = CONFIG.WEEKDAYS.map(
      (d) => `<div class="calendar-header">${d}</div>`,
    ).join("");
    ...
    for (let i = startDay - 1; i >= 0; i--)
      html += `<div class="calendar-day other-month">${prevMonth - i}</div>`;
    const today = new Date();
  ...
  selectDate(dateStr) {
    ...
    this.toast(
      `Datum gewählt: ${new Date(dateStr).toLocaleDateString(CONFIG.UI.DATE_LOCALE)}`,
      "success",
    );
  }
  prevMonth() {
  ...
  async deleteEvent(id) {
    const res = await this.apiCall(CONFIG.ENDPOINTS.EVENTS_DELETE, "POST", {
      event_id: id,
    });
    if (res?.success) {
      this.toast("Event gelöscht", "success");
      await this.loadEvents();
    }
  }
  addActivity(msg) {
...
function saveToken() {
  ...
  if (token) {
    ...
    calendar.toast("Token gespeichert", "success");
  }
}
async function importIcal() {
  const file = document.getElementById("ical-file").files[0];
  if (!file) {
    calendar.toast("Keine Datei gewählt", "warning");
    return;
  }
  const text = await file.text();
  const res = await calendar.apiCall(CONFIG.ENDPOINTS.ICAL_IMPORT, "POST", {
    ical_data: text,
  });
  if (res?.imported) {
    calendar.toast(`${res.imported} Events importiert`, "success");
