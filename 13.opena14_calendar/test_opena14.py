...
def generate_iso_datetime(days_offset=0, hours_offset=0):
    ...
    dt = datetime.now(timezone.utc) + timedelta(days=days_offset, hours=hours_offset)
...
def test_health():
    ...
    try:
        ...
        print_test("1 (Health)", "PASS", f"Uptime: {data['uptime_seconds']:.2f}s, Events: {data['total_events']}, iCal: {data['ical_support']}")
        return True
...
def test_event_filtering():
    ...
    try:
        ...
        r1 = requests.post(f"{BASE_URL}/events/create", headers=HEADERS, json={
            "calendar_id": TEST_CALENDAR_ID,
            "summary": "Past Event",
            "start": past_start,
            "end": past_end
        ...
        r2 = requests.post(f"{BASE_URL}/events/create", headers=HEADERS, json={
            "calendar_id": TEST_CALENDAR_ID,
        ...
        r3 = requests.post(f"{BASE_URL}/events/list", headers=HEADERS, json={
            "calendar_id": TEST_CALENDAR_ID,
