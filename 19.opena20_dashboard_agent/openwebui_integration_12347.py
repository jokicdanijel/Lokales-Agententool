...
async def get_system_status() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(500, f"System-Status Fehler: {str(e)}")
...
async def chat_with_openwebui(request: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        raise HTTPException(500, f"Chat-Fehler: {str(e)}")
...
async def integration_test() -> Dict[str, Any]:
    ...
    core_services_ok = all(
        ...
        if key in ["kordp_gateway", "hyper_dashboard"]
    )
...
async def get_available_models() -> Dict[str, Any]:
    ...
    try:
        ...
        async with http_session.post(
            ...
            if response.status == 200:
                ...
                return {
                    ...
                    "source": "kordp → opena3 → openwebui"
                }
            else:
                return {
                    "status": "error",
                    "error": f"kordp dispatch failed (HTTP {response.status})"
                }
        ...
        return {
            "status": "error",
            "error": str(e),
            "fallback": "Use direct OpenWebUI UI at port 8080"
        }
