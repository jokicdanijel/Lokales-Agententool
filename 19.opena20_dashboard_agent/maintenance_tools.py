        ...
        try:
            ...
            with sqlite3.connect(monitoring_db) as conn:
                ...
                if system_row:
                    system_performance = {
                        "cpu": {
                            "average": round(system_row[0] or 0, 2),
                            "peak": round(system_row[1] or 0, 2)
                        },
                        "memory": {
                            "average": round(system_row[2] or 0, 2),
                            "peak": round(system_row[3] or 0, 2)
                        },
                        "disk": {
                            "average": round(system_row[4] or 0, 2),
                            "peak": round(system_row[5] or 0, 2)
                        }
                    }
        ...
        try:
            ...
            for dir_name, info in report["disk_usage"].items():
                if info["size_mb"] > 1000:  # 1GB
                    recommendations.append(f"Consider cleaning up {dir_name} directory (currently {info['size_mb']} MB)")
            ...
            for db_name, info in report["database_health"].items():
                ...
                if isinstance(info, dict) and info.get("size_mb", 0) > 500:
                    recommendations.append(f"Large database {db_name}: {info['size_mb']} MB - consider archiving old data")
