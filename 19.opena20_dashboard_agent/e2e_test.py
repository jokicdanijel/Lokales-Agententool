...
class OpenaE2ETest:
    ...
    async def log_result(self, test_name: str, success: bool, details: str = ""):
        ...
        result = {"test": test_name, "success": success, "details": details, "timestamp": timestamp}
    ...
    def start_dashboard_if_needed(self):
        ...
        self.dashboard_process = subprocess.Popen(
            [sys.executable, "main_dashboard_final.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/health") as response:
                    if response.status == 200:
                        ...
                        success = (
                            data.get("status") == "ok"
                            and data.get("service") == "opena20"
                            and data.get("port") == 12349
                        )
                        await self.log_result(
                            "Health Endpoint", success, f"Status: {data.get('status')}, Port: {data.get('port')}"
                        )
                    else:
    ...
    async def test_authentication(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.base_url}/api/agents", headers=self.headers) as response:
                    auth_success = response.status == 200

                # Test with invalid token
                bad_headers = {"Authorization": "Bearer invalid-token"}
                async with session.get(f"{self.base_url}/api/agents", headers=bad_headers) as response:
    ...
    async def test_agent_status_aggregation(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/api/status/all", headers=self.headers) as response:
                    if response.status == 200:
                        data = await response.json()
                        success = (
                            "total" in data
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/sse/events") as response:
                    if response.status == 200:
                        ...
                        await self.log_result("SSE Events", success, f"Content-Type: {content_type}")
                    else:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    f"{self.base_url}/api/html/workflows/available", headers=self.headers
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        async with session.post(
                            f"{self.base_url}/api/html/workflows/execute", json=workflow_payload, headers=self.headers
                        ) as exec_response:
    ...
    async def test_social_media_automation(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.base_url}/api/socialmedia/status", headers=self.headers) as response:
                    status_success = response.status == 200

                # Test execution endpoint
                sm_payload = {"workflow_name": "social_media_auto_content", "platform": "test", "content_type": "test"}

                async with session.post(
                    f"{self.base_url}/api/socialmedia/execute", json=sm_payload, headers=self.headers
                ) as exec_response:
                    exec_success = exec_response.status == 200

                success = status_success and exec_success
                await self.log_result(
                    "Social Media Automation", success, f"Status: {status_success}, Execute: {exec_success}"
                )
    ...
    async def test_command_api(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(f"{self.base_url}/command", json=cmd_payload, headers=self.headers) as response:
                    if response.status == 200:
                        data = await response.json()
                        success = data.get("success") == True and "status" in data
                        await self.log_result("Command API", success, f"Success: {data.get('success')}")
                    else:
    ...
    async def test_performance_benchmark(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                await self.log_result("Performance Benchmark", success, f"10 requests in {duration:.2f}s")
