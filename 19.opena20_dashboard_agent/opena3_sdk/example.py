...
async def main():
    ...
    async with OpenA3Client(
        ...
        token=None,
        timeout=30.0,  # Add your Bearer token here
    ) as client:
        ...
        print("\n2. Native Chat:")
        try:
            response = await client.chat(prompt="Hello from SDK!", model="gpt-4", temperature=0.7)
        ...
        print("\n3. CMD Dispatch:")
        try:
            cmd = client.create_cmd_request(command="chat", payload={"prompt": "SDK CMD test"})
