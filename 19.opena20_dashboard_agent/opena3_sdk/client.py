...
from .models import ChatRequest, CMDRequest, DispatchStatus, HealthResponse, SelfTestResult
...
class OpenA3Client:
    ...
    def __init__(
        ...
        # Headers
        self.headers = {"Content-Type": "application/json", "User-Agent": "OpenA3-SDK/2.0.0"}
        if token:
            self.headers["Authorization"] = f"Bearer {token}"

        # HTTP Client
        self.client = httpx.AsyncClient(timeout=httpx.Timeout(timeout), headers=self.headers, follow_redirects=True)
    ...
    async def native_chat(self, request: ChatRequest) -> dict[str, Any]:
        ...
        return await self._request("POST", "/native", json=request.dict())
    ...
    async def cmd_dispatch(self, request: CMDRequest) -> dict[str, Any]:
        ...
        return await self._request("POST", "/cmd", json=request.dict())
    ...
    # Helper methods
    def create_cmd_request(self, command: str, payload: dict[str, Any], source: str = "sdk") -> CMDRequest:
