...
const AGENTS_CONFIG = [
    { id: 'opena1', name: 'Koordinator', kuerzel: 'kordp', port: 12344, icon: '🎯', color: 'blue' },
    { id: 'opena2', name: 'Archivator', kuerzel: 'archivp', port: 12345, icon: '📦', color: 'purple' },
    { id: 'opena3', name: 'OpenWebUI Terminal', kuerzel: 'owuip', port: 12347, icon: '🖥️', color: 'cyan' },
    { id: 'opena4', name: 'Telegram Agent', kuerzel: 'telep', port: 12348, icon: '📱', color: 'blue' },
    { id: 'opena5', name: 'VS Code Agent', kuerzel: 'vscop', port: 12351, icon: '💻', color: 'blue' },
    { id: 'opena6', name: 'Browser Agent', kuerzel: 'browsep', port: 12352, icon: '🌐', color: 'orange' },
    { id: 'opena7', name: 'Email Agent', kuerzel: 'emailp', port: 12353, icon: '📧', color: 'red' },
    { id: 'opena8', name: 'WhatsApp Agent', kuerzel: 'whatsappp', port: 12354, icon: '💬', color: 'green' },
    { id: 'opena9', name: 'Telefonie Agent', kuerzel: 'telephonep', port: 12355, icon: '📞', color: 'blue' },
    { id: 'opena10', name: 'Call Tracking Agent', kuerzel: 'calltrackp', port: 12356, icon: '📊', color: 'purple' },
    { id: 'opena11', name: 'Unlock Agent', kuerzel: 'unlockp', port: 12357, icon: '🔓', color: 'yellow' },
    { id: 'opena12', name: 'Social Media Agent', kuerzel: 'smp', port: 12358, icon: '📣', color: 'pink' },
    { id: 'opena13', name: 'Influencer Agent', kuerzel: 'influp', port: 12359, icon: '⭐', color: 'purple' },
    { id: 'opena14', name: 'Calendar Agent', kuerzel: 'calp', port: 12360, icon: '📅', color: 'green' },
    { id: 'opena15', name: 'HTML Creator', kuerzel: 'htmlp', port: 12361, icon: '🎨', color: 'pink' },
    { id: 'opena16', name: 'Shop Agent', kuerzel: 'shopp', port: 12362, icon: '🛒', color: 'orange' },
    { id: 'opena17', name: 'Homepage Creator', kuerzel: 'hpcreatep', port: 12363, icon: '🏠', color: 'blue' },
    { id: 'opena18', name: 'CRM Agent', kuerzel: 'crmp', port: 12364, icon: '👥', color: 'cyan' },
    { id: 'opena19', name: 'Stocks & Crypto', kuerzel: 'stockcryptop', port: 12365, icon: '📈', color: 'green' },
    { id: 'opena20', name: 'Dashboard', kuerzel: 'dashp', port: 12349, icon: '🚀', color: 'purple' },
];
...
class ELIONDashboard {
    ...
    renderSidebar() {
        ...
        if (!nav) return;
        nav.innerHTML = AGENTS_CONFIG.map(agent => `
    ...
    bindEvents() {
        document.getElementById('refresh-btn')?.addEventListener('click', () => this.refreshAllAgents());
        document.getElementById('start-all-btn')?.addEventListener('click', () => this.startAllAgents());
        document.getElementById('stop-all-btn')?.addEventListener('click', () => this.stopAllAgents());
        document.getElementById('api-send-btn')?.addEventListener('click', () => this.sendApiRequest());
    ...
    async checkAgentHealth(agent) {
        try {
            ...
            const res = await fetch(`http://127.0.0.1:${agent.port}/health`, { signal: controller.signal });
            clearTimeout(timeout);
            if (res.ok) {
                const data = await res.json();
                return { ...agent, online: true, data, uptime: data.uptime_seconds || 0 };
            }
        } catch (e) {}
    ...
    async refreshAllAgents() {
        const results = await Promise.all(AGENTS_CONFIG.map(a => this.checkAgentHealth(a)));
    ...
    renderAgentsGrid() {
        ...
        if (!grid) return;
        grid.innerHTML = [...this.agents.values()].map(agent => `
    ...
    async pingAgent(agentId) {
        ...
        try {
            ...
            const data = await res.json();
            this.toast(`${agent.name}: ${res.ok ? 'Online ✅' : 'Fehler'}`, res.ok ? 'success' : 'error');
            this.addActivity(res.ok ? 'success' : 'error', `Health-Check: ${agent.name}`);
    ...
    renderActivities() {
        ...
        if (!list) return;
        list.innerHTML = this.activities.slice(0, 20).map(a => `
