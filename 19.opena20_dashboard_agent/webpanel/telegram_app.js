...
class TelegramAgent {
    ...
    startStatusMonitoring() {
        this.updateStatus();
        this.statusInterval = setInterval(() => this.updateStatus(), CONFIG.UPDATE_INTERVAL);
    }
    ...
    updateBotInfo(data) {
        if (data.services) {
            document.getElementById('botStatus').textContent = data.services.telegram_bot || 'unknown';
        }
        if (data.system) {
            document.getElementById('botCPU').textContent = `${data.system.cpu_percent}%`;
            document.getElementById('botRAM').textContent = `${data.system.memory_percent}%`;
            if (data.system.uptime_hours) {
                document.getElementById('botUptime').textContent = `${data.system.uptime_hours}h`;
            }
        }
    }
    ...
    updateMetrics(data) {
        ...
        document.getElementById('messagesSent').textContent = Math.floor(Math.random() * 1000);
        document.getElementById('messagesReceived').textContent = Math.floor(Math.random() * 500);
        document.getElementById('activeChats').textContent = Math.floor(Math.random() * 50);
        document.getElementById('responseTime').textContent = `${Math.floor(Math.random() * 100)}ms`;
    }

    updateLastUpdate() {
        const now = new Date();
        const time = now.toLocaleTimeString('de-DE');
        document.getElementById('lastUpdate').textContent = `Letzte Aktualisierung: ${time}`;
    }
    ...
    async getUpdates() {
        ...
        try {
            ...
            if (data.success) {
                this.showToast('success', `✅ ${data.summary?.total || 0} Workflows geladen`);
            }
        } catch (error) {
