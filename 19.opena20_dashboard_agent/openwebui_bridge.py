...
class OpenWebUIBridge:
    ...
    async def get_dashboard_status(self) -> dict:
        ...
        try:
            ...
            return {"error": f"Dashboard connection failed: {e!s}"}
    ...
    async def get_all_agents_status(self, bearer_token: str) -> dict:
        ...
        try:
            ...
            return {"error": f"Dashboard API failed: {e!s}"}
    ...
    async def execute_workflow(self, workflow_id: str, workflow_data: dict, bearer_token: str) -> dict:
        ...
        try:
            ...
            async with self.session.post(
                f"{DASHBOARD_URL}/api/workflows/{workflow_id}", headers=headers, json=workflow_data
            ) as response:
                if response.status == 200:
                    return await response.json()
                else:
            ...
            return {"error": f"Workflow execution error: {e!s}"}
...
async def get_available_commands():
    ...
    return {
        "commands": [
            {"command": "/dashboard", "description": "Get HYPER-DASHBOARD 3.0 status", "usage": "/dashboard"},
            {"command": "/agents", "description": "List all agents status", "usage": "/agents [bearer_token]"},
            {
                "command": "/workflow",
                "description": "Execute workflow",
                "usage": "/workflow <workflow_id> [data] [bearer_token]",
            },
            {"command": "/health", "description": "Check system health", "usage": "/health"},
        ]
    }
...
def main():
    ...
    uvicorn.run(
        "openwebui_bridge:app", host="127.0.0.1", port=OPENWEBUI_PORT, reload=False, access_log=True, log_level="info"
    )
