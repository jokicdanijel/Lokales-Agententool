...
if METRICS_AVAILABLE:
    try:
        ...
        logger.info("✅ Metrics exporter initialized with {} services".format(len(AGENT_REGISTRY)))
    except Exception as e:
...
async def self_cleaning_dashboard():
    ...
    try:
        with open(BASE_DIR / "self_cleaning_dashboard.html", "r", encoding="utf-8") as f:
            html_content = f.read()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"http://127.0.0.1:{agent['port']}/health",
...
async def get_latest_safepoints(limit: int = 10, token: str = Depends(verify_token)):
    ...
    try:
        ...
        safepoint_files = sorted(
            archiv_dir.rglob("SP*.json"),
...
async def restart_agent(agent_id: str, token: str = Depends(verify_token)):
    ...
    try:
        ...
        restart_script = BASE_DIR.parent / f"{agent_id.replace('opena', '')}.{agent_id}" / "bin" / f"restart_{agent_id}.sh"
...
async def sse_events():
    ...
    async def event_generator():
        ...
        try:
            ...
            yield f"data: {json.dumps({'type': 'connected', 'timestamp': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')})}\n\n"
...
class ChatRequest(BaseModel):
    ...
    model: Optional[str] = Field(default=None, description="OpenWebUI Modell-ID")
...
async def execute_html_workflow(req: HtmlWorkflowRequest, token: str = Depends(verify_token)):
    ...
    try:
        ...
        result = fallback_results.get(req.workflow_name, {
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status == 200:
                    ...
                    html_executions = [
                        ex for ex in data.get("executions", [])
                        if ex.get("workflow_name", "").startswith("html_")
                    ]
                    
                    return {
                        "total": len(html_executions),
                        "executions": html_executions
                    }
                else:
                    raise HTTPException(status_code=resp.status, detail=f"Workflow engine error")
...
async def scan_html_systems(token: str = Depends(verify_token)):
    ...
    try:
        return await execute_html_workflow(HtmlWorkflowRequest(
...
async def deploy_html_system(
    ...
    return await execute_html_workflow(HtmlWorkflowRequest(
        workflow_name="html_deployment_pipeline",
        inputs={"system_name": name, "template": template},
        mode="async"
    ))
                ...
                try:
                    async with session.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                        health_results.append({
                            ...
                            "response_time_ms": int((resp.headers.get("X-Response-Time", "0")).replace("ms", "")) or 0
                        })
                except Exception as e:
                    health_results.append({
                        "url": url,
    ...
    try:
        ...
        for agent in status.agents:
            if metrics_exporter is not None:
                metrics_exporter.set_service_health(
                    service=agent.id,
                    port=agent.port,
                    is_up=(agent.status == "ok")
                )
...
async def execute_social_media_workflow(
    ...
    try:
        ...
        result = social_media_results.get(req.workflow_name, {
        ...
        return {
            "error": f"Social Media Workflow execution failed: {str(e)}",
            "workflow_name": req.workflow_name,
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
...
async def schedule_social_media_content(
    ...
    try:
        ...
        return {
            "error": f"Content scheduling failed: {str(e)}",
            "platform": platform,
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
...
async def start_meta_workflow(token: str = Depends(verify_token)):
    ...
    try:
        ...
        return {"error": f"Fehler beim Starten des Meta-Workflows: {str(e)}"}, 500
...
async def get_meta_workflow_status(token: str = Depends(verify_token)):
    ...
    try:
        ...
        return {"error": f"Fehler beim Abrufen des Status: {str(e)}"}, 500
...
async def html_monitoring_maintenance(token: str = Depends(verify_token)):
    ...
    try:
        ...
        monitoring_data = {
            "uptime_stats": {
                "opena15": "0%", "opena17": "99.8%", "opena6": "99.9%", "opena18": "98.5%"
            },
            "performance_metrics": {
                "avg_response_time": "245ms", "error_rate": "0.2%", "throughput": "150 req/min"
            },
            "maintenance_actions": [
...
async def trigger_health_check(token: str = Depends(verify_token)):
    ...
    try:
        ...
        health_data = {
            ...
            "services": {
                "opena20": {"running": True, "port": 12349}
            },
            "storage": {
                "free_gb": 15.7,
                "free_percent": 78.5,
