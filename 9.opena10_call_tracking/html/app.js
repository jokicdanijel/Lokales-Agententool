...
class CallTrackingDashboard {
    constructor(config) {
        ...
        this.state = {
            ...
            numbers: [],
            metrics: {
                totalCalls: 0, conversionRate: 0, activeSources: 0, activeCampaigns: 0, totalRevenue: 0, trackingNumbers: 0
            }
        };
    ...
    async init() {
        console.log(`📊 Call Tracking Dashboard v${this.config.agent.version} initializing...`);
        this.bindNavigation();
        this.bindActions();
    ...
    async loadSectionData(section) {
        switch(section) {
            ...
            case 'tracking': await this.loadTracking(); break;
            case 'sources': await this.loadSources(); break;
            case 'campaigns': await this.loadCampaigns(); break;
            case 'numbers': await this.loadNumbers(); break;
            case 'attribution': await this.loadAttribution(); break;
    ...
    async loadOverview() {
        ...
        if (!stats.error) {
            ...
            this.updateMetric('totalRevenue', `€${(stats.revenue || 0).toLocaleString('de-DE')}`);
    ...
    renderCampaigns() {
        ...
        container.innerHTML = this.state.campaigns.map(campaign => `
    ...
    renderNumbers() {
        ...
        container.innerHTML = this.state.numbers.map(num => `
    ...
    executeCapability(capabilityId) {
        switch(capabilityId) {
            case 'call_logging': this.showSection('tracking'); break;
            case 'source_tracking': this.showSection('sources'); break;
            case 'campaign_tracking': this.showSection('campaigns'); break;
            case 'dynamic_numbers': this.showSection('numbers'); break;
            case 'roi_analysis': this.showSection('analytics'); break;
            case 'custom_reports': this.showSection('reports'); break;
            default: this.showToast(`${capabilityId} aktiviert`, 'info');
        }
    }
    ...
    startAutoRefresh() {
        this.refreshInterval = setInterval(() => this.checkHealth(), this.config.ui.refreshInterval);
    }
