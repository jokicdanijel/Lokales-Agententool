...
class PermissionStore:
    ...
    def __init__(self, persist_path: str = None):
        ...
        self.persist_path = persist_path or os.getenv("PERMISSION_STORE_PATH", "data/permissions.json")
    ...
    async def grant(self, subject: str, resource: str, action: str, expires: int = 0) -> dict[str, Any]:
        """
        ...
        async with self._lock:
            ...
            for perm in self.data[subject]:
                if perm["resource"] == resource and perm["action"] == action:
                    ...
                    perm["updated_at"] = datetime.now(UTC).isoformat().replace("+00:00", "Z")
                    logger.info(f"🔄 Updated permission: {subject} -> {action} on {resource}")
                    return {"status": "updated", "subject": subject, "resource": resource, "action": action}
            ...
            return {"status": "granted", "subject": subject, "resource": resource, "action": action, "expires": expires}

    async def revoke(self, subject: str, resource: str = None, action: str = None) -> dict[str, Any]:
        """
        ...
        async with self._lock:
            ...
            self.data[subject] = [p for p in self.data[subject] if not self._matches_revoke(p, resource, action)]
            ...
            return {"status": "revoked", "removed": removed, "subject": subject}
    ...
    def search(self, resource_pattern: str = None, action: str = None) -> list[dict[str, Any]]:
        ...
        for subject, perms in self.data.items():
            for perm in perms:
                ...
                if matches:
                    results.append({"subject": subject, **perm})
