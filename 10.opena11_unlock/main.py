...
async def status():
    ...
    return {
        ...
        "audit": {"total_events": audit_log.count(), "last_event": audit_log.last_event()},
        "ai_engine": {"connected": ai_engine.is_connected(), "model": Config.OPENAI_MODEL},
        "config": {"port": Config.PORT, "dashboard_url": Config.DASHBOARD_URL},
    }
    ...
    try:
        ...
        if action == "revoke":
            result = await permission_store.revoke(
                subject=params.get("subject"), resource=params.get("resource"), action=params.get("action")
            )
        ...
        if action == "check":
            ...
            allowed = rbac_engine.check(
                subject=check_params.subject, resource=check_params.resource, action=check_params.action
            )
        ...
        if action == "ai_analyze":
            analysis = await ai_engine.analyze_permissions(
                permissions=permission_store.dump(), query=params.get("query", "Analysiere die Berechtigungsstruktur")
            )
...
async def get_logs(limit: int = 100, event_type: str | None = None):
    ...
    return {"status": "success", "total": audit_log.count(), "returned": len(logs), "logs": logs}
