...
class UnlockDashboard {
    ...
    async grantPermission() {
        ...
        const expiresHours = parseInt(document.getElementById('grant-expires').value) || 0;
        
        const expires = expiresHours > 0 
        ...
        try {
            ...
            this.showToast(`✅ Berechtigung erteilt: ${subject} → ${action} auf ${resource}`, 'success');
    ...
    async revokePermission(subject, resource, action) {
        if (!confirm(`Berechtigung widerrufen?\n${subject} → ${action} auf ${resource}`)) {
            return;
        }
    ...
    async runSecurityScan() {
        try {
            const response = await this.apiCall(CONFIG.SPECIALIZED_ENDPOINTS.SECURITY_SCAN, {
                method: 'POST'
            });
            
            this.showModal('Security Scan Ergebnis', `
                <div class="scan-result">
                    ...
                    <ul>
                        ${(response.scan?.findings || []).map(f => 
                            `<li><strong>${f.severity}:</strong> ${f.description}</li>`
                        ).join('')}
                    </ul>
                    <h5>Empfehlungen:</h5>
                    <ul>
                        ${(response.scan?.recommendations || []).map(r => 
                            `<li>${r}</li>`
                        ).join('')}
                    </ul>
                </div>
    ...
    renderPermissions() {
        ...
        const filter = document.getElementById('filter-subject')?.value?.toLowerCase() || '';
        ...
        container.innerHTML = html || '<p class="empty-state">Keine Berechtigungen</p>';
    ...
    renderAuditLog() {
        ...
        const html = this.auditLog.slice(0, CONFIG.UI.MAX_AUDIT_ENTRIES).map(entry => {
            const time = new Date(entry.timestamp).toLocaleTimeString(CONFIG.UI.DATE_LOCALE);
...
function verifyIntegrity() {
    window.dashboard?.showToast('Integritätsprüfung: Feature in Entwicklung', 'info');
}
