...
PORT = int(os.getenv("OPENA5_PORT", "12351"))
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "opena5.log", encoding="utf-8")
    ]
)
...
def mask_secrets(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: "***" if any(s in k.lower() for s in ["token", "password", "secret", "key", "bearer"]) 
            else mask_secrets(v) 
...
async def file_read(req: FileReadRequest, _: bool = Depends(verify_token)):
    ...
    try:
        ...
        # Write CMD safepoint
        write_safepoint("opena5", "kordp", "CMD", {
            "operation": "file_read",
            "path": req.path,
            "size": file_size
        }, request_id)
        ...
        write_safepoint("kordp", "opena5", "RESP", {
            "status": "success",
            "path": req.path,
            "size": file_size,
...
async def file_write(req: FileWriteRequest, _: bool = Depends(verify_token)):
    ...
    try:
        ...
        if full_path.suffix not in ALLOWED_EXTENSIONS:
            raise HTTPException(
                status_code=400,
                detail=f"Extension '{full_path.suffix}' not allowed. Allowed: {ALLOWED_EXTENSIONS}"
            )
        ...
        if content_size > MAX_FILE_SIZE:
            raise HTTPException(
                status_code=413,
                detail=f"Content too large: {content_size} bytes (max: {MAX_FILE_SIZE})"
            )
