...
class VSCodeIntegration:
    ...
    async def get_workspace_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"workspace_root": self.workspace_root, "error": str(e), "status": "error"}
    ...
    async def edit_file(self, file_path: str, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"action": "edit_file", "file_path": file_path, "error": str(e), "success": False}
    ...
    async def create_project(self, project_type: str, project_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(project_path):
                return {"action": "create_project", "error": f"Project {project_name} already exists", "success": False}
            ...
            return {"action": "create_project", "error": str(e), "success": False}
    ...
    async def git_commit(self, message: str, files: list[str] = None) -> dict[str, Any]:
        ...
        try:
            ...
            return {"action": "git_commit", "error": f"Git error: {e.stderr}", "success": False}
        except Exception as e:
    ...
    async def analyze_code(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not os.path.exists(full_path):
                return {"action": "analyze_code", "error": f"File not found: {file_path}", "success": False}
            ...
            return {"action": "analyze_code", "analysis": analysis, "success": True}

        except Exception as e:
            return {"action": "analyze_code", "error": str(e), "success": False}
    ...
    async def _create_react_project(self, path: str, name: str):
        ...
        package_json = {
            ...
            "version": "1.0.0",
            "description": "React project created by ELION VSCode Agent",
            "main": "index.js",
            "scripts": {"start": "react-scripts start", "build": "react-scripts build"},
            "dependencies": {"react": "^18.0.0", "react-dom": "^18.0.0", "react-scripts": "5.0.1"},
        }
...
async def handle_command(request: CommandRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        await publish_sse_event(
            "command_received", {"agent": Config.AGENT_NAME, "command": request.command, "target": request.target}
        )
        ...
        if request.command == "vscode_status":
            ...
            result = await vscode_integration.edit_file(
                request.params.get("file_path", ""), request.params.get("content", "")
            )
        elif request.command == "create_project":
            result = await vscode_integration.create_project(
                request.params.get("project_type", "generic"), request.params.get("project_name", "new_project")
            )
        else:
...
if __name__ == "__main__":
    uvicorn.run("main_opena5:app", host="127.0.0.1", port=Config.AGENT_PORT, reload=False, log_level="info")
