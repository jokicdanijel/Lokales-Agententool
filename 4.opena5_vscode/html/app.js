...
class VSCodeDashboard {
  ...
  updateMetrics(data) {
    ...
    document.getElementById("metricExtensions").textContent =
      data.extensions || 0;
  }
  ...
  loadCapabilities() {
    ...
    if (!grid) return;
    grid.innerHTML = this.config.capabilities
      .map(
        (c) => `
  ...
  async testCapability(id) {
    this.showToast(`Testing ${id}...`, "info");
  }
  ...
  async loadFileTree() {
    ...
    if (!container) return;
    try {
      const data = await this.apiCall("/vscode/files").catch(() => ({
        files: [],
      }));
  ...
  openFileFromTree(name) {
    this.showToast(`Opening ${name}`, "info");
  }
  searchFiles() {
  ...
  async formatCode() {
    ...
    if (editor) {
      try {
        const res = await this.apiCall("/vscode/format", "POST", {
          code: editor.value,
          language: "python",
        });
  ...
  async runCode() {
    this.showToast("Running code...", "info");
  }
  ...
  async executeTerminal() {
    ...
    try {
      const res = await this.apiCall("/vscode/terminal", "POST", {
        command: cmd,
      });
  ...
  clearTerminal() {
    const output = document.getElementById("terminalOutput");
    if (output)
      output.innerHTML = '<div class="term-line">$ Terminal cleared</div>';
  }

  // Git
  async gitRefresh() {
    try {
      const status = await this.apiCall("/vscode/git", "POST", {
        action: "status",
      }).catch(() => ({ output: "No changes" }));
      const log = await this.apiCall("/vscode/git", "POST", {
        action: "log",
      }).catch(() => ({ commits: [] }));

      document.getElementById("gitStatusOutput").textContent =
        status.output || "Clean";
      document.getElementById("gitLogOutput").innerHTML =
        (log.commits || [])
          .slice(0, 5)
          .map(
            (c) =>
  ...
  async gitPull() {
    this.showToast("Pulling...", "info");
  }
  async gitPush() {
    this.showToast("Pushing...", "info");
  }
  async gitCommit() {
    this.showToast("Commit dialog...", "info");
  }
  async gitBranch() {
    this.showToast("Branches...", "info");
  }
  async gitStash() {
    this.showToast("Stashing...", "info");
  }
  async gitDiff() {
    this.showToast("Diff view...", "info");
  }
  gitStatus() {
    this.showSection("git");
  }

  // Debug
  startDebug() {
    this.showToast("Debug started", "success");
  }
  stopDebug() {
    this.showToast("Debug stopped", "info");
  }
  ...
  async explainCode() {
    ...
    try {
      const res = await this.apiCall("/vscode/analyze", "POST", {
        code,
        action: "explain",
      });
      document.getElementById("explanationOutput").textContent =
        res.explanation || res.result || "No explanation";
    } catch {
      this.showToast("AI failed", "error");
    }
  }
  ...
  async refactorCode() {
    ...
    try {
      ...
      document.getElementById("refactorOutput").textContent =
        res.refactored || res.result || code;
    } catch {
      this.showToast("Refactor failed", "error");
    }
  }
  ...
  async generateTests() {
    ...
    try {
      ...
      document.getElementById("testsOutput").textContent =
        res.tests || res.result || "# Generated tests...";
    } catch {
  ...
  installExtension() {
    this.showToast("Extension marketplace...", "info");
  }
  ...
  openFile() {
    this.showSection("filetree");
  }
  createFile() {
    this.showToast("Create file dialog...", "info");
  }
  runTerminal() {
