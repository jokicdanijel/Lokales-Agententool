...
def test_command():
    ...
    try:
        payload = {"request_id": "test_12345", "command": "test_command", "payload": {"msg": "Hello from opena4 test"}}
...
def test_invalid_json():
    ...
    try:
        payload = {"request_id": "test_12346", "command": "test", "extra_field": "should_be_rejected"}  # Nicht erlaubt
        resp = requests.post(f"{BASE_URL}/command", json=payload, headers=HEADERS, timeout=5)
...
def test_safepoints():
    ...
    try:
        ...
        for line in recent:
            ...
            print(
                f"   - {entry.get('sp', 'N/A')} | {entry.get('src', 'N/A')} → {entry.get('dst', 'N/A')} | {entry.get('kind', 'N/A')}"
            )
