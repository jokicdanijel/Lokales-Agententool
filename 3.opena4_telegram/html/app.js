...
class TelegramDashboard {
    ...
    bindFormEvents() {
        ...
        // Media Send Form
        document.getElementById('mediaSendForm')?.addEventListener('submit', (e) => {
            e.preventDefault();
            this.sendMedia();
        });
        ...
        // Webhook Config Form
        document.getElementById('webhookConfigForm')?.addEventListener('submit', (e) => {
            e.preventDefault();
    ...
    async checkHealth() {
        try {
            ...
            this.setOnlineStatus(result.status === 'healthy' || result.status === 'ok');
            return result;
        } catch (error) {
    ...
    async sendBulkMessage() {
        const chatIds = document.getElementById('bulkChatIds')?.value.split('\n').filter(id => id.trim());
        ...
        if (chatIds.length > this.config.telegram.rateLimits.bulkLimit) {
            this.showToast(`Max. ${this.config.telegram.rateLimits.bulkLimit} Empfänger erlaubt`, 'warning');
            return;
        }
        ...
        try {
            ...
            this.showToast(`Bulk-Nachricht an ${chatIds.length} Empfänger gestartet`, 'success');
    ...
    async loadInbox() {
        ...
        try {
            ...
            if (result.messages && result.messages.length > 0) {
                inboxList.innerHTML = result.messages.map(msg => `
                ...
                inboxList.innerHTML = '<div class="inbox-empty">Keine neuen Nachrichten</div>';
            }
        } catch (error) {
    ...
    async sendMedia() {
        ...
        try {
            const response = await fetch(`${this.config.api.baseUrl}${this.config.api.endpoints.uploadMedia}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem(this.config.storage.bearerToken) || ''}`
                },
            ...
            document.getElementById('uploadZone').innerHTML = '<span>📤</span><p>Drag & Drop oder klicken</p>';
        } catch (error) {
            this.showToast(`Fehler: ${error.message}`, 'error');
        }
    }
    ...
    async loadMediaGallery() {
        ...
        try {
            ...
            if (result.items && result.items.length > 0) {
                gallery.innerHTML = result.items.map(item => `
    ...
    saveContacts() {
        localStorage.setItem(this.config.storage.contacts, JSON.stringify(this.contacts));
    }
        ...
        try {
            const result = await this.apiCall(
                ...
                { 
                    message: input,
                    settings: {
                        model: document.getElementById('aiModel')?.value || this.config.ai.model,
                        temperature: parseFloat(document.getElementById('aiTemperature')?.value) || this.config.ai.temperature,
                        max_tokens: parseInt(document.getElementById('aiMaxTokens')?.value) || this.config.ai.maxTokens
                    }
                }
            );
    ...
    async clearContext() {
        try {
            ...
            document.getElementById('contextDisplay').innerHTML = '<p>Kontext gelöscht</p>';
        } catch (error) {
            this.showToast(`Fehler: ${error.message}`, 'error');
        }
    }
    ...
    saveTemplate() {
        ...
        const variables = document.getElementById('templateVariables')?.value.split(',').map(v => v.trim()).filter(v => v);
        ...
        localStorage.setItem(this.config.storage.templates, JSON.stringify(this.templates));
        this.renderTemplates();
    ...
    async loadWebhookStatus() {
        try {
            const result = await this.apiCall(this.config.api.endpoints.webhookStatus);
            
            document.getElementById('webhookUrl').textContent = result.url || 'Nicht konfiguriert';
            document.getElementById('webhookActive').textContent = result.active ? 'Aktiv' : 'Inaktiv';
            document.getElementById('webhookLastEvent').textContent = result.last_event || '-';
            document.getElementById('webhookPending').textContent = result.pending_updates || '0';
    ...
    async updateWebhookConfig() {
        ...
        const allowedUpdates = Array.from(document.querySelectorAll('input[name="allowedUpdates"]:checked'))
            .map(cb => cb.value);

        try {
            await this.apiCall(
                this.config.api.endpoints.webhookConfig,
                'POST',
                { url, secret, allowed_updates: allowedUpdates }
            );
    ...
    async loadAnalytics() {
        try {
            const result = await this.apiCall(this.config.api.endpoints.analyticsOverview);
            
            // Update metric cards
            document.getElementById('analyticsSent')?.textContent && 
                (document.getElementById('analyticsSent').textContent = result.messages_sent || 0);
            document.getElementById('analyticsReceived')?.textContent && 
                (document.getElementById('analyticsReceived').textContent = result.messages_received || 0);
            document.getElementById('analyticsActiveChats')?.textContent && 
                (document.getElementById('analyticsActiveChats').textContent = result.active_chats || 0);
            document.getElementById('analyticsResponseTime')?.textContent && 
                (document.getElementById('analyticsResponseTime').textContent = (result.avg_response_time || 0) + 'ms');
    ...
    async loadTopUsers() {
        ...
        try {
            ...
            if (result.users && result.users.length > 0) {
                list.innerHTML = result.users.map((user, index) => `
    ...
    async loadMessageHistory() {
        ...
        try {
            const result = await this.apiCall(this.config.api.endpoints.messageHistory);
            if (result.messages && result.messages.length > 0) {
    ...
    async processRetryQueue() {
        ...
        for (let i = this.retryQueue.length - 1; i >= 0; i--) {
            ...
            try {
                await new Promise(resolve => setTimeout(resolve, this.config.api.retryDelay * item.attempts));
                await this.apiCall(item.endpoint, item.method, item.data);
                this.log('info', `Retry erfolgreich: ${item.endpoint}`);
                this.retryQueue.splice(i, 1);
            } catch (error) {
                this.log('warning', `Retry ${item.attempts}/${item.maxAttempts} fehlgeschlagen: ${item.endpoint}`);
            }
        }
    }
        ...
        try {
            ...
            if (metrics) {
                document.getElementById('metricSent')?.textContent && 
                    (document.getElementById('metricSent').textContent = metrics.messages_sent || 0);
                document.getElementById('metricReceived')?.textContent && 
                    (document.getElementById('metricReceived').textContent = metrics.messages_received || 0);
                document.getElementById('metricChats')?.textContent && 
                    (document.getElementById('metricChats').textContent = metrics.active_chats || 0);
                document.getElementById('metricQueue')?.textContent && 
                    (document.getElementById('metricQueue').textContent = metrics.queue_size || 0);
            }
    ...
    async saveSettings() {
        ...
        const rateMsgPerSec = parseInt(document.getElementById('rateMsgPerSec')?.value);
        const rateMsgPerMin = parseInt(document.getElementById('rateMsgPerMin')?.value);
    ...
    downloadLogs() {
        const data = this.logs.map(l => `${l.time} [${l.level}] ${l.message}`).join('\n');
...
function sendQuickMessage() {
    ...
    if (chatId && message) {
        dashboard.apiCall(CONFIG.api.endpoints.sendMessage, 'POST', { chat_id: chatId, text: message })
