...
async def register_with_portier():
    ...
    registration_data = {
        ...
        "capabilities": ["telegram_messaging", "mobile_notifications", "chat_management", "media_handling"],
        "endpoints": ["/health", "/api/send_message", "/api/get_updates", "/api/chat_history", "/metrics"],
        "status": "active",
...
async def health():
    ...
    return HealthResponse(telegram_connected=telegram_bot is not None, portier_mode=config.PORTIER_MODE)


@app.post("/api/send_message", response_model=TelegramResponse)
async def send_message(message: TelegramMessage, background_tasks: BackgroundTasks, token: str = Depends(verify_token)):
    ...
    try:
        sent_message = await telegram_bot.send_message(
            chat_id=message.chat_id, text=message.text, parse_mode=message.parse_mode
        )
        ...
        response_data = {"status": "sent", "message_id": sent_message.message_id}

        # Archive in background
        background_tasks.add_task(archive_to_opena2, message.dict(), response_data)
...
async def execute_command(
    command: CommandRequest, background_tasks: BackgroundTasks, token: str = Depends(verify_token)
):
    ...
    try:
        ...
        response_data = {"status": "success", "result": result, "execution_time_ms": execution_time}

        # Archive in background
        background_tasks.add_task(archive_to_opena2, command.dict(), response_data)
        ...
        raise HTTPException(500, f"Command failed: {e!s}")
...
async def process_command(command: str, parameters: dict) -> Any:
    ...
    if command == "get_bot_info":
        if telegram_bot:
            bot_info = await telegram_bot.get_me()
            return {"username": bot_info.username, "first_name": bot_info.first_name, "id": bot_info.id}
...
if __name__ == "__main__":
    ...
    logger.info("🔥 PORTIER 3.0 - opena4_telegram Enterprise Agent")
