...
def write_safepoint(
    ...
    logger.debug("Safepoint written: %s", sp_path)
    return sp_path
...
async def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if credentials.credentials != config.bearer_token:
        logger.warning("❌ Ungültiger Token: %s***", credentials.credentials[:10])
...
async def command_endpoint(req: Request, _: bool = Depends(verify_token)):
    ...
    try:
        ...
        logger.info("Command received: %s (ID: %s)", command, request_id)
...
async def receive_telegram_message(req: Request):
    """Receive Telegram message (webhook or polling → HTTP injection)."""
    try:
        data = await req.json()
        ...
        if config.telegram_allowed_users and msg_data.user_id not in config.telegram_allowed_users:
            write_safepoint(
                ...
                "opena4",
                "ERR",
                {"error": "unauthorized_user", "user_id": msg_data.user_id},
        ...
        try:
            ...
            return JSONResponse({"ok": True, "request_id": request_id, "response": resp_data})
        ...
        error_resp = create_error_response_83(
            ...
            details={
                "validation_errors": [
                    {
                        "field": (err.get("loc") or ["unknown"])[0],
                        "message": str(err.get("msg", "Unknown error")),
                    }
                    for err in e.errors()
                ]
            },
...
def main():
    ...
    parser.add_argument(
        "--telegram-polling",
        action="store_true",
        help="Use Telegram polling instead of webhook",
    )
    parser.add_argument(
        "--no-telegram",
        action="store_true",
        help="Disable Telegram bot (HTTP only)",
    )

    args = parser.parse_args()

    # Override config if provided at runtime
    if args.port != config.port:
        config.port = args.port
    if args.host != config.host:
        config.host = args.host

    global telegram_app, telegram_mode

    logger.info("Starting opena4 @ %s:%s", args.host, args.port)
    ...
    if not args.no_telegram and config.telegram_bot_token:
        ...
        telegram_mode = "disabled"
        logger.info("Telegram bot disabled (no token or --no-telegram)")

    # Start FastAPI server (blocking)
    logger.info("Starting FastAPI server @ %s:%s", args.host, args.port)
    uvicorn.run(
        app,
        host=args.host,
        port=args.port,
