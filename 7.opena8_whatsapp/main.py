...
# Global instances
core: WhatsAppCore | None = None
api: WhatsAppAPI | None = None
ai_engine: AIReplyEngine | None = None
...
async def webhook_verification(request: Request):
    ...
    try:
        ...
        # Verify token
        if hub_mode == "subscribe" and hub_verify_token == config.META_WEBHOOK_VERIFY_TOKEN:
...
async def archive_message(message: WhatsAppMessage):
    ...
    try:
        ...
        archive_data = {
            ...
            "output_data": {"processed_at": datetime.now().isoformat(), "status": message.status},
            "metadata": {"direction": "inbound", "timestamp": message.timestamp.isoformat()},
        }
...
async def send_auto_reply(message: WhatsAppMessage):
    ...
    try:
        ...
        success, msg_id, error = await whatsapp_client.send_message(message.phone_number, reply_text)
...
async def send_message_api(request: SendMessageRequest):
    ...
    try:
        success, message_id, error = await whatsapp_client.send_message(request.to_phone, request.body)

        if success:
            return {"status": "sent", "message_id": message_id, "to_phone": request.to_phone}
        else:
...
async def get_messages():
    ...
    return {"total": len(message_store), "messages": message_store[-50:]}  # Last 50 messages
