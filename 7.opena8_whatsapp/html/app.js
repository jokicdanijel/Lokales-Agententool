...
class WhatsAppDashboard {
    constructor(config) {
        ...
        this.state = {
            ...
            metrics: { sent: 0, received: 0, pending: 0, failed: 0, contacts: 0, templates: 0 }
        };
        this.refreshInterval = null;
    }

    async init() {
        console.log(`🟢 WhatsApp Agent Dashboard v${this.config.agent.version} initializing...`);
        this.bindNavigation();
        this.bindActions();
        this.bindModals();
    ...
    async loadSectionData(section) {
        switch(section) {
            ...
            case 'inbox': await this.loadConversations(); break;
            ...
            case 'webhooks': await this.loadWebhooks(); break;
            case 'analytics': await this.loadAnalytics(); break;
        }
    }

    bindActions() {
        document.getElementById('sendMessageBtn')?.addEventListener('click', () => this.showComposeModal());
        ...
        document.getElementById('registerWebhookBtn')?.addEventListener('click', () => this.showWebhookModal());
    ...
    bindModals() {
        ...
        // Compose Modal
        document.getElementById('sendComposeBtn')?.addEventListener('click', () => this.sendComposedMessage());
        document.getElementById('attachMediaBtn')?.addEventListener('click', () => this.attachMedia());
    }
    ...
    async loadMessages(convId) {
        const result = await this.apiCall(`${this.config.api.endpoints.conversationHistory}?id=${convId}`);
    ...
    async syncContacts() {
        ...
        const result = await this.apiCall(this.config.api.endpoints.syncContacts, 'POST');
        if (!result.error) {
            this.showToast(`${result.synced || 0} Kontakte synchronisiert`, 'success');
    ...
    async sendComposedMessage() {
        ...
        const result = await this.apiCall(this.config.api.endpoints.sendMessage, 'POST', {
            to: recipient,
            message: message
        });
    ...
    renderMediaGallery() {
        ...
        container.innerHTML = this.state.media.map(item => `
    ...
    renderTemplates() {
        ...
        container.innerHTML = this.state.templates.map(tpl => `
    ...
    renderWebhooks() {
        ...
        if (this.state.webhooks.length === 0) {
            container.innerHTML = '<div class="loading">Keine Webhooks registriert</div>';
            return;
        }

        container.innerHTML = this.state.webhooks.map(wh => `
    ...
    async registerWebhook() {
        ...
        const result = await this.apiCall(this.config.api.endpoints.webhookRegister, 'POST', { url, events });
        if (!result.error) {
    ...
    async deleteWebhook(id) {
        const result = await this.apiCall(`${this.config.api.endpoints.webhookDelete}?id=${id}`, 'DELETE');
        if (!result.error) {
    ...
    renderAnalytics() {
        ...
        if (data.daily) {
            ...
            this.updateMetric('analyticsDeliveryRate', `${data.daily.deliveryRate || 0}%`);
    ...
    executeCapability(capabilityId) {
        ...
        switch(capabilityId) {
            case 'incoming_messages': this.showSection('inbox'); break;
            case 'outgoing_messages': this.showComposeModal(); break;
            case 'media_messages': this.showMediaModal(); break;
            case 'contact_manager': this.showSection('contacts'); break;
            case 'template_api': this.showSection('templates'); break;
            case 'webhook_listener': this.showSection('webhooks'); break;
            case 'conversation_history': this.showSection('inbox'); break;
            case 'account_health': this.checkHealth(); break;
            default: this.showToast(`${capability.name} aktiviert`, 'info');
        }
    }
    ...
    async sendBroadcast() {
        const recipients = document.getElementById('broadcastRecipients')?.value.split('\n').filter(r => r.trim());
        ...
        this.showToast(`${success}/${recipients.length} gesendet`, success === recipients.length ? 'success' : 'warning');
        this.closeModals();
    }
