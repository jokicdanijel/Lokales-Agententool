    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.post(
                ...
                json={"agent_id": "opena8", "endpoint": f"http://127.0.0.1:{config.PORT}", "component": "whatsapp"},
            )
...
async def webhook_handler(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Archive to opena2
        safepoint = Safepoint(ts=datetime.now(UTC), src="opena8", dst="opena2", kind="MSG", payload=msg_obj.dict())

        # Background: archive safepoint
        background_tasks.add_task(_archive_safepoint, safepoint)
    ...
    try:
        ...
        if req.message_type == MessageType.TEXT:
            ...
            success, msg_id, error = await wa_client.send_media(
                req.to_phone, req.media_url, req.media_type.value if req.media_type else "image"
            )
        ...
        return SendMessageResponse(success=success, message_id=msg_id, error=error, sent_at=datetime.now(UTC))
...
async def run_action(req: MailRunRequest):
    ...
    try:
        ...
        if action == "ingest":
            ...
            if msg_obj:
                return MailRunResponse(success=True, action="ingest", data=msg_obj.dict(), timestamp=datetime.now(UTC))

        elif action == "send":
            # Send message
            success, msg_id, error = await wa_client.send_message(payload.get("to_phone"), payload.get("body", ""))
        ...
        return MailRunResponse(
            success=False, action=action, error=f"Unknown action: {action}", timestamp=datetime.now(UTC)
        )
...
async def _archive_safepoint(safepoint: Safepoint):
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.post(f"{config.OPENA2_URL}/store/archivp", json=safepoint.dict())
            if response.status_code == 200:
