...
class OpenaConfig(BaseSettings):
    ...
    MEDIA_TYPES: list[str] = ["image", "document", "audio", "video"]
    ...
    # Security & Filtering
    WHATSAPP_ALLOWLIST: list[str] = [s.strip() for s in os.getenv("WHATSAPP_ALLOWLIST", "").split(",") if s.strip()]
    WHATSAPP_BLOCKLIST: list[str] = [s.strip() for s in os.getenv("WHATSAPP_BLOCKLIST", "").split(",") if s.strip()]
