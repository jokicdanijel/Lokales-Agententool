...
def test_match_influencers():
    ...
    try:
        payload = {"campaign_id": TEST_CAMPAIGN_ID, "max_results": 5, "min_score": 50.0}
        ...
        print_test(
            "7 (Match Influencers)", "PASS", f"Matches: {len(data['matches'])}, Candidates: {data['total_candidates']}"
        )
        return True
    except Exception as e:
...
def test_metrics():
    """Test 8: Get Metrics"""
    try:
        payload = {"platform": "instagram", "niche": "fashion"}
        ...
        print_test(
            "8 (Metrics)", "PASS", f"Profiles: {data['total_profiles']}, Avg Engagement: {data['avg_engagement_rate']}%"
        )
        return True
    except Exception as e:
...
def test_matching_algorithm_edge_cases():
    ...
    try:
        ...
        match_payload = {"campaign_id": campaign_id, "max_results": 10, "min_score": 50.0}
        ...
        assert (
            "MicroInfluencer_HighEngagement" not in matched_names
        ), "Edge case failed: Low followers should be filtered out"
