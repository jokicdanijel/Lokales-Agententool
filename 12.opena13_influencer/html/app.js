class InfluencerDashboard {
    ...
    bindEvents() {
        document.getElementById('profile-form')?.addEventListener('submit', e => this.createProfile(e));
        document.getElementById('campaign-form')?.addEventListener('submit', e => this.createCampaign(e));
        document.getElementById('match-form')?.addEventListener('submit', e => this.runMatching(e));
    }
    ...
    async loadHealth() {
        ...
        if (data?.status === 'ok') {
            ...
            document.getElementById('total-profiles').textContent = data.total_profiles || 0;
    ...
    async createProfile(e) {
        ...
        const profile = {
            ...
            followers: parseInt(document.getElementById('profile-followers').value) || 0,
            engagement_rate: parseFloat(document.getElementById('profile-engagement').value) || 0,
            niche: document.getElementById('profile-niche').value,
            contact_email: document.getElementById('profile-email').value
        };
        const res = await this.apiCall(CONFIG.ENDPOINTS.PROFILES_CREATE, 'POST', profile);
        if (res?.profile_id) {
    ...
    async createCampaign(e) {
        ...
        const campaign = {
            ...
            min_followers: parseInt(document.getElementById('campaign-min-followers').value) || 0,
        ...
        const res = await this.apiCall(CONFIG.ENDPOINTS.CAMPAIGNS_CREATE, 'POST', campaign);
        if (res?.campaign_id) {
    ...
    async runMatching(e) {
        ...
        if (!campaignId) { this.toast('Bitte Kampagne wählen', 'warning'); return; }
        const matchReq = {
            campaign_id: campaignId,
            max_results: parseInt(document.getElementById('match-max').value) || 10,
            min_score: parseFloat(document.getElementById('match-min-score').value) || 60
        };
        ...
        if (res?.matches) {
            ...
            this.addActivity(`Matching durchgeführt: ${res.matches.length} Ergebnisse`);
        } else this.toast('Keine Matches gefunden', 'warning');
    }
    renderProfiles() {
        const container = document.getElementById('profiles-list');
        document.getElementById('profiles-count').textContent = this.profiles.length;
        if (!this.profiles.length) { container.innerHTML = '<p class="empty-state">Keine Profile</p>'; return; }
        container.innerHTML = this.profiles.slice(0, 20).map(p => `
    ...
    renderMatches(matches) {
        ...
        if (!matches.length) { container.innerHTML = '<p class="empty-state">Keine Matches</p>'; return; }
    ...
    updatePlatformMetrics() {
        ...
        let totalEng = 0;
        this.profiles.forEach(p => { counts[p.platform] = (counts[p.platform] || 0) + 1; totalEng += p.engagement_rate || 0; });
        document.getElementById('instagram-count').textContent = counts.instagram || 0;
        ...
        document.getElementById('linkedin-count').textContent = counts.linkedin || 0;
        document.getElementById('avg-engagement').textContent = this.profiles.length ? (totalEng / this.profiles.length).toFixed(1) + '%' : '0%';
        document.getElementById('total-reach').textContent = this.formatNumber(this.profiles.reduce((s, p) => s + (p.followers || 0), 0));
    }
    async deleteProfile(id) {
        const res = await this.apiCall(CONFIG.ENDPOINTS.PROFILES_DELETE, 'POST', { profile_id: id });
        if (res?.success) { this.toast('Profil gelöscht', 'success'); await this.loadProfiles(); await this.loadHealth(); }
    }
    addActivity(msg) {
    ...
    formatNumber(n) { return n >= 1e6 ? (n/1e6).toFixed(1)+'M' : n >= 1e3 ? (n/1e3).toFixed(1)+'K' : n.toString(); }
    updateUptime() {
...
function saveToken() {
    ...
    if (token) { localStorage.setItem(CONFIG.AUTH.TOKEN_KEY, token); dashboard.toast('Token gespeichert', 'success'); }
}
