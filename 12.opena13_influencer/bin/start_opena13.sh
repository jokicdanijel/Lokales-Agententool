#!/usr/bin/env bash
#
# start_opena13.sh - Start opena13 (Influencer Management Agent)
# ================================================================
#
# Agent:   opena13
# Port:    12359
...
check_dependencies() {
    ...
    python3 -c "import fastapi, uvicorn, pydantic" 2>/dev/null && {
        log_info "✅ Alle Dependencies vorhanden"
        return 0
    }

    log_info "⚠️  Dependencies fehlen, versuche Installation..."

    # Try installation with --break-system-packages for externally-managed environments
    pip3 install --break-system-packages fastapi uvicorn pydantic 2>/dev/null || {
        log_error "❌ pip install fehlgeschlagen (PEP 668 externally-managed environment)"
        log_error "Verwende bestehende System-Pakete oder richte venv ein"

        # Final check: Are packages available system-wide?
        python3 -c "import fastapi, uvicorn, pydantic" 2>/dev/null || {
...
check_running() {
    if [[ -f "$PID_FILE" ]]; then
        ...
        if ps -p "$PID" >/dev/null 2>&1; then
            log_error "❌ opena13 läuft bereits (PID: $PID)"
            exit 1
        else
...
start_service() {
    log_info "Starte opena13 (Port $PORT)..."
...
main() {
    log_info "=== opena13 (Influencer) START ==="
