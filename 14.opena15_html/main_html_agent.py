...
class HTMLProcessor:
    ...
    @staticmethod
    def inject_css_framework(html: str, framework: CSSFramework, custom_css: Optional[str] = None) -> str:
    ...
    @staticmethod
    def inject_meta_tags(html: str, title: str, description: Optional[str], keywords: Optional[List[str]]) -> str:
...
class TemplateManager:
    ...
    def render_template(template_name: str, variables: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"Template syntax error: {str(e)}")
        except Exception as e:
...
async def preview_html(req: PreviewRequest, _: bool = Depends(verify_token)):
    ...
    if head:
        viewport_meta = soup.new_tag("meta", attrs={
            "name": "viewport",
            "content": f"width={req.width}, initial-scale=1.0"
        })
    ...
    try:
        if req.format == "html":
            ...
            return ExportResponse(
                success=True,
                file_path=str(file_path),
                message=f"HTML exported to {file_path}"
            )
            ...
            return ExportResponse(
                ...
                message="HTML encoded to Base64"
            )
            ...
            return ExportResponse(
                ...
                message=f"HTML exported to ZIP: {zip_path}"
            )
