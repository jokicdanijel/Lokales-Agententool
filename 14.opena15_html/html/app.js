class HTMLCreatorDashboard {
    constructor() {
        ...
        this.history = [];
        this.stats = { bootstrap: 0, tailwind: 0, bulma: 0, plain: 0, valid: 0, invalid: 0, exports: 0, totalSize: 0, count: 0 };
    ...
    bindEvents() {
        document.getElementById('generate-form')?.addEventListener('submit', e => this.generateHTML(e));
        document.getElementById('validate-form')?.addEventListener('submit', e => this.validateHTML(e));
        document.getElementById('preview-form')?.addEventListener('submit', e => this.previewHTML(e));
        document.getElementById('export-form')?.addEventListener('submit', e => this.exportHTML(e));
    }
    ...
    async loadHealth() {
        ...
        if (data?.status === 'ok') {
            ...
            document.getElementById('total-templates').textContent = data.templates_available || this.templates.length;
        } else {
    ...
    async generateHTML(e) {
        ...
        const payload = {
            ...
            keywords: document.getElementById('meta-keywords').value.split(',').map(k => k.trim()).filter(k => k)
        };
        ...
        if (res?.html || res?.success) {
            ...
            this.addActivity(`HTML generiert: ${payload.template_name} (${payload.css_framework})`);
            document.getElementById('total-generated').textContent = parseInt(document.getElementById('total-generated').textContent) + 1;
            if (res.html) {
    ...
    async validateHTML(e) {
        ...
        if (!html.trim()) { this.toast('Bitte HTML eingeben', 'warning'); return; }
        const payload = { html, validation_level: document.getElementById('validation-level').value };
        ...
        document.getElementById('total-validations').textContent = this.stats.valid + this.stats.invalid;
        this.updateStats();
        this.addActivity(`Validierung: ${res?.valid || res?.is_valid ? 'OK' : 'Fehler'} (${payload.validation_level})`);
    }
    async previewHTML(e) {
        e.preventDefault();
        ...
        const payload = {
            html,
            width: parseInt(document.getElementById('preview-width').value) || CONFIG.PREVIEW.DEFAULT_WIDTH,
            height: parseInt(document.getElementById('preview-height').value) || CONFIG.PREVIEW.DEFAULT_HEIGHT
        };
    ...
    async exportHTML(e) {
        ...
        const payload = {
            html,
            filename: document.getElementById('export-filename').value || 'export.html',
    ...
    updateStats() {
        document.getElementById('stat-bootstrap').textContent = this.stats.bootstrap;
        ...
        document.getElementById('stat-total-exports').textContent = this.stats.exports;
        const avgSize = this.stats.count > 0 ? (this.stats.totalSize / this.stats.count / 1024).toFixed(1) : 0;
    ...
    renderHistory() {
        ...
        if (!this.history.length) { container.innerHTML = '<p class="empty-state">Kein Verlauf</p>'; return; }
        const typeLabels = { generate: '🚀 Generate', validate: '✅ Validate', export: '💾 Export' };
...
function saveToken() {
    ...
    if (token) { localStorage.setItem(CONFIG.AUTH.TOKEN_KEY, token); htmlCreator.toast('Token gespeichert', 'success'); }
}
