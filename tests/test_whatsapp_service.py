...
from app.whatsapp_client import MediaHandler, MessageClassifier, WhatsAppClient
...
class TestMessageClassifier:
    ...
    def test_sentiment_classification_urgent(self):
        ...
        sentiment, urgency = MessageClassifier.classify_sentiment("URGENT: Help needed immediately", "")
        assert sentiment == SentimentType.URGENT
        assert urgency >= 7

    def test_sentiment_classification_positive(self):
        """Classify POSITIVE sentiment"""
        sentiment, urgency = MessageClassifier.classify_sentiment("Great job, thank you so much", "")
...
class TestWhatsAppMessage:
    ...
    def test_message_creation_with_media(self):
        ...
        media = MediaObject(media_type=MediaType.IMAGE, media_id="img123", mime_type="image/jpeg")
        msg = WhatsAppMessage(
            message_id="msg456",
            phone_number="+49123456789",
            timestamp=datetime.now(UTC),
...
class TestSendMessageRequest:
    ...
    def test_text_message_request(self):
        ...
        req = SendMessageRequest(to_phone="+49123456789", message_type=MessageType.TEXT, body="Test message")
...
class TestSafepoint:
    ...
    def test_safepoint_creation(self):
        """Create safepoint"""
        sp = Safepoint(ts=datetime.now(UTC), src="opena8", dst="opena2", kind="MSG", payload={"message_id": "msg123"})
...
class TestMailRunRequests:
    ...
    def test_ingest_action(self):
        ...
        req = MailRunRequest(action="ingest", payload={"webhook_entry": {}})
        assert req.action == "ingest"

    def test_send_action(self):
        """Test send action request"""
        req = MailRunRequest(action="send", payload={"to_phone": "+49123456789", "body": "Hi"})
...
class TestMailRunResponse:
    ...
    def test_success_response(self):
        ...
        resp = MailRunResponse(success=True, action="send", data={"message_id": "msg123"}, timestamp=datetime.now(UTC))
        assert resp.success is True

    def test_error_response(self):
        """Test error response"""
        resp = MailRunResponse(success=False, action="send", error="Invalid phone number", timestamp=datetime.now(UTC))
