...
from app.browser_client import PolicyGate
...
class TestHealthEndpoints:
    ...
    def test_readiness_response_structure(self):
        ...
        ready = ReadyResponse(ready=True, browser="playwright-chromium", version="1.0.0")
...
class TestPolicyGate:
    ...
    async def test_domain_allowlist_check(self):
        ...
        compliance = ComplianceConfig(allow_domains=["example.org", "localhost"], obey_robots=True)

        gate = PolicyGate(compliance)

        # Should allow
        request = PlaybookRequest(
            steps=[PlaybookStep(action=ActionType.GOTO, url="https://example.org")], compliance=compliance
        )
    ...
    async def test_domain_blocklist_enforcement(self):
        ...
        compliance = ComplianceConfig(allow_domains=["example.org"], obey_robots=True)

        gate = PolicyGate(compliance)

        # Should block
        request = PlaybookRequest(
            steps=[PlaybookStep(action=ActionType.GOTO, url="https://evil.com")], compliance=compliance
        )
...
class TestPlaybookResponse:
    ...
    def test_error_response_structure(self):
        ...
        from app.models import ArtifactCollection, ErrorInfo, TimingInfo
...
class TestSafepoint:
    ...
    def test_safepoint_creation(self):
        ...
        payload = {"status": "success", "artifacts": {"screenshots": []}}
    ...
    async def test_simple_navigation(self):
        ...
        try:
            request = PlaybookRequest(
                ...
                steps=[
                    PlaybookStep(action=ActionType.GOTO, url="https://example.org", wait="load"),
                    PlaybookStep(action=ActionType.SCREENSHOT, label="homepage"),
                ],
                compliance=ComplianceConfig(allow_domains=["example.org"]),
            )
...
class TestMockExecutor:
    ...
    async def test_mock_playbook_execution(self):
        ...
        from app.models import ArtifactCollection, ArtifactRef, TimingInfo
        ...
        mock_response = PlaybookResponse(
            ...
            status="success",
            artifacts=ArtifactCollection(
                screenshots=[ArtifactRef(label="homepage", path="archivp/2025/11/10/screenshot.png", sha256="abc123")]
            ),
