...
class TestOpsScript:
    ...
    def test_ops_script_syntax(self):
        ...
        result = subprocess.run(["bash", "-n", "bin/ops.sh"], capture_output=True, text=True)
    ...
    def test_ops_help_command(self):
        ...
        result = subprocess.run(["bash", "bin/ops.sh", "help"], capture_output=True, text=True, timeout=10)
    ...
    def test_ops_start_validates_openai_key(self):
        ...
        result = subprocess.run(
            ["timeout", "5", "bash", "bin/ops.sh", "start"], capture_output=True, text=True, timeout=10
        )
...
class TestRegisterAgentsScript:
    ...
    def test_register_script_has_shebang(self):
        ...
        assert first_line.startswith("#!/usr/bin/env python3"), f"Invalid shebang: {first_line}"
...
class TestEnvironmentConfiguration:
    ...
    def test_env_has_dashboard_token(self):
        ...
        assert "DASHBOARD_ADMIN_TOKEN=" in content, "DASHBOARD_ADMIN_TOKEN not found in .env"

    def test_env_has_openai_key(self):
        """Verify .env has OPENAI_API_KEY."""
        env_file = Path(".env")
        content = env_file.read_text()
        assert "OPENAI_API_KEY=" in content, "OPENAI_API_KEY not found in .env"
...
class TestScriptsIntegration:
    ...
    def test_ops_can_call_register_script(self):
        ...
        assert "register_agents.py" in ops_content, "ops.sh doesn't reference register_agents.py"

    def test_ops_agents_register_command_exists(self):
        """Verify bin/ops.sh has agents:register command."""
        ops_content = Path("bin/ops.sh").read_text()
        assert "agents:register" in ops_content, "ops.sh doesn't have agents:register command"
