...
def test_services_folder_structure():
  ...
  for svc, target in EXPECTED.items():
    ...
    has_target = (f"program_target: {target}" in txt or 
                  f"program_target**: `{target}`" in txt or
                  f'program_target: "{target}"' in txt or
...
def test_readme_exists_for_all_services():
  ...
  core_services = [
    "portier", "openwebui", "telegram", "vscode", "browser",
    "calendar_agent", "chatbot_email", "chatbot_whats", "chatbot_tone_answer",
    "chatbot_tone_call", "unlock_master", "social_media", "influencer",
    "html_creator", "shop_creator", "homepage_creator", "local_archiv_agent",
    "stocks_crypto", "dashboard_agent"
  ]
