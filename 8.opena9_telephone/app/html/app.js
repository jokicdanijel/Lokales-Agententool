...
class TelephoneDashboard {
  ...
  async fetchActiveCalls() {
    try {
      const data = await this.apiCall(
        CONFIG.SPECIALIZED_ENDPOINTS.ACTIVE_CALLS,
      );
  ...
  async makeCall() {
    ...
    try {
      const response = await this.apiCall(
        CONFIG.SPECIALIZED_ENDPOINTS.MAKE_CALL,
        {
          method: "POST",
          body: JSON.stringify({
            to: phoneNumber,
  ...
  async generateVoice() {
    ...
    try {
      const response = await this.apiCall(
        CONFIG.SPECIALIZED_ENDPOINTS.VOICE_GENERATE,
        {
          method: "POST",
          body: JSON.stringify({
            text: text,
      ...
      this.showToast(
        `Voice-Generierung fehlgeschlagen: ${error.message}`,
        "error",
      );
    }
  }
  ...
  updateMetricsDisplay(data) {
    ...
    this.setElementText(
      "avg-duration",
      `${Math.round(timings.average_call_duration_seconds || 0)}s`,
    );
    this.setElementText(
      "voice-generations",
      counters.voice_responses_generated || 0,
    );
    this.setElementText(
      "transcriptions",
      counters.transcriptions_completed || 0,
    );
    this.setElementText(
      "api-response",
      `${Math.round(timings.average_response_time_ms || 0)}ms`,
    );
    this.setElementText("error-rate", `${(rates.error_rate || 0).toFixed(1)}%`);
    this.setElementText(
      "calls-per-hour",
      (rates.calls_per_hour || 0).toFixed(1),
    );
  }
  ...
  updateActiveCallsDisplay() {
    ...
    container.innerHTML = this.activeCalls
      .map(
        (call) => `
            <div class="call-item">
                <div class="call-info">
