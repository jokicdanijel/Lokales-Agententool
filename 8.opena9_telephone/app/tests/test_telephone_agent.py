...
class TestTelephonyCore:
    ...
    async def test_execute_make_call(self):
        ...
        core = TelephonyCore()
        result = await core.execute("make_call", {"to": "+491234567890", "from_number": "+490987654321"})
...
class TestTelephonyAPI:
    ...
    async def test_make_call_mock(self):
        ...
        api = TelephonyAPI()
        result = await api.make_call(to="+491234567890", from_number="+490987654321")
...
class TestAIVoiceEngine:
    ...
    async def test_generate_ivr_flow_mock(self):
        ...
        engine = AIVoiceEngine()
        result = await engine.generate_ivr_flow(scenario="customer_support", options=["sales", "support", "billing"])
    ...
    async def test_auto_response(self):
        ...
        engine = AIVoiceEngine()
        result = await engine.auto_response(trigger="incoming_call", caller_id="+491234567890")
...
class TestMetrics:
    ...
    def test_metrics_record_call(self):
        ...
        metrics.record_call({"call_id": "test-123", "direction": "outbound", "duration": 60, "status": "completed"})
...
class TestMainApp:
    ...
    def test_command_endpoint(self, client):
        ...
        response = client.post("/command", json={"command": "list_active_calls", "params": {}})
        assert response.status_code == 200
