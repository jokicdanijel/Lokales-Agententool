...
class CommandRequest(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                "command": "make_call",
                "args": {"to": "+49123456789", "script": "Hello, this is an automated call from Telephone Agent 6.0"},
            }
        }
...
class SpecializedRequest(BaseModel):
    ...
    text: str | None = Field(None, description="Text for TTS generation")
    call_id: str | None = Field(None, description="Call ID for context")
...
async def execute_command(
    request: CommandRequest, background_tasks: BackgroundTasks, token: str = Depends(verify_token)
) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Command execution failed: {e!s}")


@app.post("/specialized", tags=["ai"])
async def ai_specialized(
    request: SpecializedRequest, background_tasks: BackgroundTasks, token: str = Depends(verify_token)
) -> dict[str, Any]:
    ...
    try:
        ...
        result = await ai_engine.handle_specialized_request(
            request.action, {"text": request.text, "call_id": request.call_id, **request.context}
        )
        ...
        raise HTTPException(status_code=400, detail=f"AI function failed: {e!s}")
...
async def get_config(token: str = Depends(verify_token)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Configuration retrieval failed: {e!s}")
