...
class TelephonyAPI:
    ...
    async def initialize(self):
        ...
        if self.account_sid and self.auth_token:
            ...
            self.session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30), auth=auth)
    ...
    async def make_call(self, to: str, script: str = "", caller_id: str = None) -> dict[str, Any]:
        ...
        twiml = (
            self._generate_twiml(script) if script else "<Response><Say>Hello from Telephone Agent 6.0</Say></Response>"
        )

        if self.session and self.account_sid:
            try:
                payload = {"To": to, "From": from_number, "Twiml": twiml}

                async with self.session.post(f"{self.twilio_base_url}/Calls.json", data=payload) as response:
...
<Response>
    ...
    async def hangup(self, call_id: str) -> dict[str, Any]:
        ...
        if self.session and self.account_sid and call_id.startswith("CA"):
            try:
                ...
                async with self.session.post(f"{self.twilio_base_url}/Calls/{call_id}.json", data=payload) as response:
    ...
    async def call_status(self, call_id: str) -> dict[str, Any]:
        ...
        if self.session and self.account_sid and call_id.startswith("CA"):
            try:
                async with self.session.get(f"{self.twilio_base_url}/Calls/{call_id}.json") as response:
                    if response.status == 200:
                        data = await response.json()
                        return {
                            "call_id": call_id,
        ...
        return {"call_id": call_id, "status": "unknown", "timestamp": datetime.now().isoformat()}
    ...
    async def send_dtmf(self, call_id: str, digits: str) -> dict[str, Any]:
        ...
        if self.session and self.account_sid and call_id.startswith("CA"):
            try:
                ...
                async with self.session.post(f"{self.twilio_base_url}/Calls/{call_id}.json", data=payload) as response:
                    if response.status == 200:
                        return {"call_id": call_id, "action": "send_dtmf", "digits": digits, "status": "sent"}
            except Exception as e:
                logger.error(f"DTMF send failed: {e}")

        return {"call_id": call_id, "action": "send_dtmf", "digits": digits, "status": "sent", "provider": "mock"}
    ...
    async def play_audio(self, call_id: str, audio_url: str) -> dict[str, Any]:
        ...
        if self.session and self.account_sid and call_id.startswith("CA"):
            try:
                ...
                async with self.session.post(f"{self.twilio_base_url}/Calls/{call_id}.json", data=payload) as response:
                    if response.status == 200:
                        return {"call_id": call_id, "action": "play_audio", "audio_url": audio_url, "status": "playing"}
            except Exception as e:
    ...
    async def get_call_history(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        if self.session and self.account_sid:
            try:
                async with self.session.get(f"{self.twilio_base_url}/Calls.json?PageSize={limit}") as response:
                    if response.status == 200:
                        data = await response.json()
