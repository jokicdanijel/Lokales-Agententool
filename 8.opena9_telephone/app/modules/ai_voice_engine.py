...
class AIVoiceEngine:
    ...
    def __init__(self):
        ...
        self.ivr_templates = {
            ...
            "press_1": {
                "id": "press_1",
                "text": "Für Verkauf, drücken Sie 1.",
                "action": "transfer_sales"
            },
            "press_2": {
                "id": "press_2", 
                "text": "Für Support, drücken Sie 2.",
                "action": "transfer_support"
            },
            "press_0": {
                "id": "press_0",
...
Generate a natural, conversational response that would be spoken by a virtual assistant."""
                ...
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are a professional phone assistant. Generate natural, spoken responses."},
...
Format as JSON with: welcome_message, options (array with number, text, action)"""
                ...
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an IVR system designer. Generate structured phone menu flows."},
                        {"role": "user", "content": prompt}
                    ],
                    max_tokens=800,
                    temperature=0.5
        ...
        if self.client:
            try:
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "Analyze the sentiment of this call transcript. Return JSON with: sentiment (positive/neutral/negative), confidence (0-1), emotions (list), summary."},
        ...
        if self.client and (intent or caller_message):
            try:
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are an AI phone assistant. Generate brief, helpful responses."},
        ...
        if self.client:
            try:
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "Classify the caller's intent. Return JSON with: intent (inquiry/complaint/sales/support/other), confidence (0-1), keywords."},
                        {"role": "user", "content": transcript}
                    ],
                    max_tokens=200,
                    temperature=0.3
