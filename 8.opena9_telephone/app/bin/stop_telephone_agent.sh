#!/bin/bash
# ====================================================
# 📞 OPENA9 Telephone Agent - Stop Script
# PORTIER PAS-6.0
# ====================================================
...
echo -e "${YELLOW}📞 Stopping OPENA9 Telephone Agent${NC}"

if [ ! -f "$PID_FILE" ]; then
    echo -e "${YELLOW}⚠️  No PID file found - agent may not be running${NC}"
    
    # Try to find and kill by port
    PID=$(lsof -ti:12355 2>/dev/null || true)
    if [ -n "$PID" ]; then
        echo -e "${YELLOW}Found process on port 12355 (PID: $PID)${NC}"
...
if kill -0 "$PID" 2>/dev/null; then
    ...
    rm -f "$PID_FILE"
    echo -e "${GREEN}✅ OPENA9 Telephone Agent stopped${NC}"
else
