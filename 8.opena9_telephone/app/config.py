...
class OpenaConfig(BaseSettings):
    ...
    FORWARD_TO_NUMBER: Optional[str] = os.getenv("FORWARD_TO_NUMBER")
    ...
    ALLOWED_NUMBERS: List[str] = [
        s.strip() 
        for s in os.getenv("ALLOWED_NUMBERS", "").split(",")
        if s.strip()
    ]
    BLOCKED_NUMBERS: List[str] = [
        s.strip() 
        for s in os.getenv("BLOCKED_NUMBERS", "").split(",")
        if s.strip()
    ]
    ...
    PROVIDER_WEBHOOK_SECRET: Optional[str] = os.getenv("PROVIDER_WEBHOOK_SECRET")
