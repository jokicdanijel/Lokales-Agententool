...
from app.sip_client import SIPClient, CallManager
from app.models import CallRecord, CallDirection, CallStatus
...
# FastAPI App
app = FastAPI(
    title="opena9 Telephone Agent",
    description="VoIP/SIP Integration für Telefonie",
    version="1.0.0"
)
...
class MakeCallRequest(BaseModel):
    ...
    duration_limit: Optional[int] = 300  # 5 Minuten default
...
async def get_call_history(limit: int = 50):
    ...
    return {
        ...
        "calls": call_history[-limit:] if call_history else []
    }
...
async def get_stats():
    ...
    recent_calls = [
        ...
        if datetime.fromisoformat(call.get("started_at", "2000-01-01")) > day_ago
    ]
