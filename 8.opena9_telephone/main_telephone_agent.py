...
def mask_secrets(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: (
                "***"
                if any(s in k.lower() for s in ["token", "password", "secret", "credential", "sid", "auth"])
                else mask_secrets(v)
            )
...
class TwilioClient:
    ...
    def start_call(self, to: str, from_number: str | None = None, timeout: int = 60) -> dict[str, Any]:
        """Initiate outbound call"""
        if not self.account_sid or not self.auth_token:
            raise HTTPException(
                status_code=500, detail="TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN not configured in .env"
            )
        ...
        payload = {"To": to, "From": from_num, "Url": twiml_url, "Timeout": timeout}

        try:
            response = requests.post(url, data=payload, auth=(self.account_sid, self.auth_token), timeout=10)
            ...
            raise HTTPException(status_code=502, detail=f"Twilio API error: {e!s}")
    ...
    def get_call_status(self, call_sid: str) -> dict[str, Any]:
        ...
        try:
            response = requests.get(url, auth=(self.account_sid, self.auth_token), timeout=5)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.HTTPError as e:
    ...
    def hangup_call(self, call_sid: str) -> dict[str, Any]:
        ...
        try:
            response = requests.post(
                url, data={"Status": "completed"}, auth=(self.account_sid, self.auth_token), timeout=5
            )
            response.raise_for_status()
            return response.json()
        except requests.exceptions.HTTPError as e:
...
class CallStartRequest(BaseModel):
    ...
    from_number: str | None = Field(None, description="Caller ID (optional, uses default)")
...
app = FastAPI(title="opena9 Telefonie Agent", description="SIP/Twilio Call Management", version="1.0.0")
...
async def startup():
    ...
    logger.info(
        f"   Twilio Account SID: {TWILIO_ACCOUNT_SID[:8]}***"
        if TWILIO_ACCOUNT_SID
        else "   Twilio Account SID: NOT CONFIGURED"
    )
...
async def root():
    ...
    return {
        ...
        "port": PORT,
        "status": "running",
        "capabilities": ["call/start", "call/hangup", "call/status", "webhook/status"],
        "telephony": {
            "provider": "twilio",
...
async def call_start(req: CallStartRequest, _: bool = Depends(verify_token)):
    ...
    write_safepoint(
        ...
        "twilio_api",
        "CMD",
        {"action": "call_start", "to": req.to, "from": req.from_number or TWILIO_PHONE_NUMBER, "timeout": req.timeout},
        request_id,
    )
...
async def call_hangup(req: CallHangupRequest, _: bool = Depends(verify_token)):
    ...
    # Safepoint CMD
    write_safepoint(KUERZEL, "twilio_api", "CMD", {"action": "call_hangup", "call_id": req.call_id}, request_id)
    ...
    try:
        ...
        if req.call_id in active_calls:
            ...
            active_calls[req.call_id]["end_time"] = datetime.now(UTC).isoformat().replace("+00:00", "Z")
...
async def call_status(call_id: str, _: bool = Depends(verify_token)):
    ...
    if call_id in active_calls:
        return {"call_id": call_id, **active_calls[call_id]}
...
async def webhook_status(request: Request, x_twilio_signature: str | None = Header(None, alias="X-Twilio-Signature")):
    ...
    if call_sid in active_calls:
        ...
        if call_status in ["completed", "failed", "busy", "no-answer"]:
            active_calls[call_sid]["end_time"] = datetime.now(UTC).isoformat().replace("+00:00", "Z")
