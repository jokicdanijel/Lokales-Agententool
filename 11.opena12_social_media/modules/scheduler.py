        ...
        if self.queue_file.exists():
            try:
                with open(self.queue_file) as f:
                    data = json.load(f)
                    self.queue = [ScheduledPost(**item) for item in data.get("queue", [])]
                    self.history = [ScheduledPost(**item) for item in data.get("history", [])[-50:]]
    ...
    def cancel(self, job_id: str) -> dict[str, Any]:
        ...
        for i, post in enumerate(self.queue):
            if post.job_id == job_id:
                ...
                return {"status": "success", "message": f"Post {job_id} cancelled", "job_id": job_id}
    ...
    def get_ready_posts(self) -> list[ScheduledPost]:
        ...
        for post in self.queue:
            if post.status == "scheduled":
                try:
                    scheduled = datetime.fromisoformat(post.scheduled_at.replace("Z", "+00:00")).replace(tzinfo=None)
        ...
        for post in ready_posts:
            try:
                ...
                result = await self.core.post_now(
                    platforms=post.platforms, text=post.text, hashtags=post.hashtags, media=post.media
                )
                ...
                results.append({"job_id": post.job_id, "status": "executed", "result": result})
                ...
                results.append({"job_id": post.job_id, "status": "failed", "error": str(e)})
