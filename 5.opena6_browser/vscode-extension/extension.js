...
async function connectToAgent() {
    ...
    try {
        const health = await api.health();
        outputChannel.appendLine(`✅ Connected to ${health.agent || 'opena5_vscode'} agent`);
        
        const status = await api.status();
        outputChannel.appendLine(`📊 Agent Status: ${JSON.stringify(status, null, 2)}`);
        
        return true;
    } catch (error) {
        outputChannel.appendLine(`❌ Failed to connect: ${error.message}`);
        vscode.window.showErrorMessage(`PORTIER Connection Failed: ${error.message}`);
        return false;
    }
}
        ...
        vscode.commands.registerCommand("portier.runCommand", async () => {
            ...
            if (cmd) {
                try {
                    ...
                    outputChannel.appendLine(`📥 Response: ${JSON.stringify(response, null, 2)}`);
                    vscode.window.showInformationMessage("PORTIER Command executed successfully");
                } catch (error) {
        ...
        vscode.commands.registerCommand("portier.analyzeFile", async () => {
            ...
            try {
                vscode.window.withProgress({
                    ...
                    outputChannel.appendLine(`🔍 Analysis Result: ${JSON.stringify(response, null, 2)}`);
            ...
            try {
                vscode.window.withProgress({
                    ...
                    if (response.output || response.refactored_code) {
                        ...
                        vscode.window.showWarningMessage("No refactoring suggestions available");
                    }
                });
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("portier.explain", async () => {
            ...
            try {
                ...
                const explanation = response.explanation || response.result || "No explanation available";
        ...
        vscode.commands.registerCommand("portier.createFile", async () => {
            ...
            if (description) {
                try {
                    ...
                    vscode.window.showErrorMessage(`File creation failed: ${error.message}`);
                }
            }
        })
    );
