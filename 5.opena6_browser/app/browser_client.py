...
class PolicyGate:
    ...
    async def validate_request(self, request: PlaybookRequest) -> tuple[bool, str | None]:
        """
...
class ArtifactWriter:
    ...
    async def capture_screenshot(self, page: Page, label: str, full_page: bool = False) -> ArtifactRef | None:
        ...
        try:
            ...
            filename = f"screenshot_{label}_{datetime.now(UTC).timestamp():.0f}.png"
            ...
            return ArtifactRef(
                label=label, path=str(filepath), sha256=sha256, size_bytes=size_bytes, mime_type="image/png"
            )
        except Exception as e:
            logger.error(f"Failed to capture screenshot: {e}")
            return None

    async def capture_html(self, page: Page, label: str) -> ArtifactRef | None:
        ...
        try:
            ...
            filename = f"page_{label}_{datetime.now(UTC).timestamp():.0f}.html"
            filepath = artifact_dir / filename
            ...
            return ArtifactRef(
                label=label, path=str(filepath), sha256=sha256, size_bytes=size_bytes, mime_type="text/html"
            )
        except Exception as e:
            logger.error(f"Failed to capture HTML: {e}")
            return None

    async def capture_har(self, page: Page, label: str) -> ArtifactRef | None:
        ...
        try:
            ...
            filename = f"session_{label}_{datetime.now(UTC).timestamp():.0f}.har"
            filepath = artifact_dir / filename

            # Placeholder HAR structure
            har_data = {"log": {"version": "1.2", "creator": {"name": "opena6", "version": "1.0"}, "entries": []}}
            ...
            return ArtifactRef(
                label=label, path=str(filepath), sha256=sha256, size_bytes=size_bytes, mime_type="application/json"
            )
        except Exception as e:
        ...
        try:
            ...
            for step_idx, step in enumerate(request.steps):
                ...
                try:
                    ...
                    error_elapsed = int((datetime.now(UTC) - step_start).total_seconds() * 1000)
                    event_logs.append(
                        EventLog(
            ...
            # Success response
            total_elapsed = int((datetime.now(UTC) - start_time).total_seconds() * 1000)
    ...
    async def _execute_step(
        ...
        try:
            if step.action == ActionType.GOTO:
                ...
                ref = await self.artifact_writer.capture_screenshot(
                    page, step.label or "screenshot", step.full_page or False
                )
                if ref:
                    artifacts.screenshots.append(ref)
            ...
            # Log success
            step_elapsed = int((datetime.now(UTC) - step_start).total_seconds() * 1000)
            event_logs.append(
                EventLog(
                    ts=datetime.now(UTC).isoformat().replace("+00:00", "Z"),
                    request_id=request.request_id,
