...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/opena6.nohup.log"),
...
class NavigateRequest(BaseModel):
    ...
    wait_until: str = Field("load", description="When to consider navigation done (load, domcontentloaded, networkidle)")
...
class ExtractRequest(BaseModel):
    ...
    wait_for: Optional[str] = Field(None, description="Selector to wait for before extraction")
    timeout: int = Field(DEFAULT_TIMEOUT, description="Timeout in milliseconds")
...
class FormFillRequest(BaseModel):
    ...
    submit_selector: Optional[str] = Field(None, description="Submit button selector")
...
def mask_secrets(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: "***" if any(s in k.lower() for s in ["token", "password", "secret", "key", "bearer", "auth"])
            else mask_secrets(v)
...
async def navigate(req: NavigateRequest, _: bool = Depends(verify_token)):
    ...
    write_safepoint("browsep", "kordp", "CMD", {
        "endpoint": "/navigate",
        "url": req.url,
