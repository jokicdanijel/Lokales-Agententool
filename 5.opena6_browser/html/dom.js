...
function toggleDOMInspector() {
  ...
  if (domInspectorEnabled) {
    ...
    showNotification(
      "DOM Inspector activated - Click elements to inspect",
      "info",
    );
  } else {
...
function renderDOMTree(htmlString) {
  ...
  try {
    ...
    domTree.innerHTML =
      '<div class="error">Failed to render DOM structure</div>';
  }
}
...
function buildDOMTreeHTML(element, depth) {
  ...
  if (children.length > 0) {
    ...
    for (const child of children.slice(0, 50)) {
      // Limit to 50 children for performance
...
async function showElementInfo(selector) {
  ...
  try {
    ...
    const response = await api(
      `/dom/element?selector=${encodeURIComponent(selector)}`,
    );
...
async function getElementText() {
  ...
  try {
    ...
    log(
      "spec_output",
      `Text from ${selectedElement}: ${response.result || "No text"}`,
    );
  } catch (error) {
