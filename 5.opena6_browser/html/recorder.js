...
function updateActionCount() {
    ...
    if (actionCount) {
        actionCount.textContent = currentRecording ? currentRecording.actions.length : 0;
    }
}
...
window.api = async function(path, method = 'GET', payload = null) {
    ...
    if (isRecording && (method === 'POST' || method === 'PUT')) {
        ...
        if (path === '/command' && payload) {
            ...
            actionData = { ...actionData, command: payload.command, args: payload.args };
        } else if (path === '/specialized' && payload) {
            actionType = payload.action || 'specialized';
            actionData = { ...actionData, action: payload.action, source: payload.source };
        }
...
function updateRecordingsList() {
    ...
    if (recordings.length === 0) {
        recordingsList.innerHTML = '<div class="text-muted">No recordings available</div>';
        return;
    }
...
async function playRecording(recordingId) {
    ...
    for (const action of recording.actions) {
        try {
            ...
            const nextAction = recording.actions[recording.actions.indexOf(action) + 1];
            if (nextAction) {
                const delay = Math.min(nextAction.relativeTime - action.relativeTime, 5000); // Max 5s delay
                await new Promise(resolve => setTimeout(resolve, Math.max(delay, 100))); // Min 100ms delay
            }
...
function initializeTimeline() {
    ...
    timelineContainer.innerHTML = '<canvas id="timelineCanvas" width="800" height="200"></canvas>';
...
function updateTimelineChart() {
    ...
    actions.forEach((action, index) => {
        ...
        // Draw action label
        if (actions.length < 20) { // Only show labels if not too crowded
            ctx.fillStyle = '#fff';
