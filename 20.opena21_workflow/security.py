...
async def verify_token(credentials: HTTPAuthorizationCredentials | None = Depends(security)) -> str:
    ...
    if credentials.credentials != BEARER_TOKEN:
        logger.warning("Ungültiger Token-Versuch")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
class WorkflowSecurityManager:
    ...
    ALLOWED_ACTIONS = {"call_agent", "transform_data", "condition", "wait", "parallel", "sequence", "retry", "timeout"}
    ...
    DANGEROUS_PATTERNS = {"__import__", "eval(", "exec(", "compile(", "os.system", "subprocess", "shell=True"}
    ...
    def sanitize_params(cls, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in params.items():
            if isinstance(value, str):
                ...
                sanitized[key] = [cls.sanitize_params(v) if isinstance(v, dict) else v for v in value]
            else:
                sanitized[key] = value
        return sanitized

    @classmethod
    def validate_workflow_definition(cls, workflow: dict[str, Any]) -> tuple[bool, str | None]:
        """
...
def log_workflow_event(event_type: str, workflow_id: str, details: dict[str, Any] | None = None) -> None:
    """
