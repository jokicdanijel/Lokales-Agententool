...
class StepDefinition(BaseModel):
    ...
    agent: str | None = Field(None, description="Ziel-Agent (opena3-opena20, kordp)")
    ...
    condition: str | None = Field(None, description="Bedingung für Step-Ausführung (JSON-Path)")
    on_failure: str | None = Field("stop", description="Verhalten bei Fehler (stop|continue|retry)")
    depends_on: list[str] | None = Field(None, description="Step-Abhängigkeiten")
...
class WorkflowDefinition(BaseModel):
    ...
    description: str | None = Field(None, description="Workflow-Beschreibung")
...
class WorkflowStatus(BaseModel):
    ...
    current_step: str | None = Field(None, description="Aktueller Step (wenn running)")
    started_at: str = Field(..., description="Start-Zeitstempel (ISO 8601)")
    completed_at: str | None = Field(None, description="Completion-Zeitstempel")
    outputs: dict[str, Any] = Field(default_factory=dict, description="Workflow-Outputs")
    error: str | None = Field(None, description="Fehlermeldung (wenn failed)")
...
class SafepointRequest(BaseModel):
    ...
    workflow_id: str | None = Field(None, description="Zugehörige Workflow-ID")
    step_name: str | None = Field(None, description="Zugehöriger Step-Name")
...
async def call_agent_via_kordp(agent: str, action: str, params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        dispatch_payload = {"service_target": agent, "action": action, "params": params}
...
async def execute_step(step: StepDefinition, workflow_id: str, context: dict[str, Any]) -> dict[str, Any]:
    ...
    cmd_safepoint = SafepointRequest(
        ...
        kind="CMD",
        payload={"workflow_id": workflow_id, "step_name": step.name, "action": step.action, "params": step.params},
    ...
    try:
        ...
        error_safepoint = SafepointRequest(
            ...
            destination="error",
            kind="RESP",
            payload={"workflow_id": workflow_id, "step_name": step.name, "success": False, "error": str(e)},
            workflow_id=workflow_id,
            step_name=step.name,
        )
...
async def execute_workflow_async(workflow: WorkflowDefinition, inputs: dict[str, Any]) -> WorkflowStatus:
    ...
    try:
        ...
        status_obj.completed_at = datetime.now(UTC).isoformat()
...
async def invoke(request: InvokeRequest, token: str = Depends(verify_token)):
    """
    ...
    if request.action == "execute_workflow":
        ...
        if not workflow_name or workflow_name not in workflows_registry:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Workflow '{workflow_name}' nicht gefunden"
            )

        workflow = workflows_registry[workflow_name]
        status_obj = execute_workflow_sync(workflow, inputs)

        return {"status": "success", "action": request.action, "result": status_obj.model_dump()}
        ...
        if not workflow_id or workflow_id not in executions_registry:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Workflow-ID '{workflow_id}' nicht gefunden"
            )
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Unbekannte Action: {request.action}")


@app.post("/workflows/create", tags=["Workflows"])
async def create_workflow(workflow: WorkflowDefinition, token: str = Depends(verify_token)):
    """Erstellt eine neue Workflow-Definition"""
    if workflow.name in workflows_registry:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail=f"Workflow '{workflow.name}' existiert bereits"
        )

    workflows_registry[workflow.name] = workflow

    return {"status": "success", "message": f"Workflow '{workflow.name}' erstellt", "workflow": workflow.model_dump()}
...
async def execute_workflow_endpoint(request: ExecuteRequest, token: str = Depends(verify_token)):
    """Führt einen Workflow aus mit PORTIER 3.0 Integration"""
    if request.workflow_name not in workflows_registry:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Workflow '{request.workflow_name}' nicht gefunden"
        )
    ...
    start_safepoint = SafepointRequest(
        ...
        kind="CMD",
        payload={"workflow_name": request.workflow_name, "inputs": request.inputs, "mode": request.mode},
    )
    await send_safepoint(start_safepoint)

    if request.mode == "sync":
        status_obj = await execute_workflow_async(workflow, request.inputs)
        return {"status": "success", "execution": status_obj.model_dump()}
    else:
...
async def get_workflow_status(workflow_id: str, token: str = Depends(verify_token)):
    ...
    if workflow_id not in executions_registry:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Workflow-ID '{workflow_id}' nicht gefunden")
...
async def cancel_workflow(workflow_id: str, token: str = Depends(verify_token)):
    ...
    cancel_safepoint = SafepointRequest(
        ...
        destination="cancel",
        kind="CMD",
        payload={"workflow_id": workflow_id, "action": "cancel", "reason": "User requested cancellation"},
        workflow_id=workflow_id,
    )
    await send_safepoint(cancel_safepoint)

    return {"status": "success", "message": f"Workflow {workflow_id} abgebrochen", "execution": status_obj.model_dump()}
...
async def get_statistics(token: str = Depends(verify_token)):
    ...
    return {
        "status": "success",
        "statistics": workflow_stats,
        "workflows": {"total_defined": len(workflows_registry), "workflow_names": list(workflows_registry.keys())},
        "executions": {
            ...
            steps=[
                StepDefinition(
                    ...
                    params={
                        ...
                        "alert_thresholds": {"response_time": 3000, "error_rate": 0.01, "uptime": 0.995},
                    },
                    timeout=30,
                ),
                StepDefinition(
