...
from main import (
    ...
    WorkflowDefinition,
    WorkflowState,
    call_agent_via_kordp,
    check_portier_connection,
    execute_step,
...
class TestWorkflowEngine:
    ...
    def test_workflow_definition_validation(self):
        ...
        workflow = WorkflowDefinition(
            ...
            steps=[
                StepDefinition(name="step1", action="call_agent", agent="opena3", params={"test": "value"}, timeout=30)
            ],
            timeout=120,
        )
    ...
    def test_execute_request_validation(self):
        ...
        request = ExecuteRequest(workflow_name="demo_workflow", inputs={"param1": "value1"}, mode="sync")
...
class TestWorkflowExecution:
    ...
    async def test_execute_step_call_agent(self):
        ...
        step = StepDefinition(name="test_step", action="call_agent", agent="opena3", params={"query": "test query"})

        workflow_id = "test_workflow_123"
        context = {}

        with patch("main.call_agent_via_kordp") as mock_call_agent, patch("main.send_safepoint") as mock_safepoint:
    ...
    async def test_execute_step_transform_data(self):
        ...
        step = StepDefinition(
            name="transform_step", action="transform_data", params={"input": "raw_data", "format": "json"}
        )
    ...
    async def test_execute_workflow_success(self):
        ...
        workflow = WorkflowDefinition(
            ...
            description="Test Workflow",
            steps=[
                StepDefinition(name="step1", action="transform_data", params={"data": "test1"}),
                StepDefinition(name="step2", action="transform_data", params={"data": "test2"}),
            ],
            timeout=60,
        )
    ...
    async def test_execute_workflow_failure(self):
        ...
        workflow = WorkflowDefinition(
            ...
            steps=[StepDefinition(name="failing_step", action="call_agent", agent="nonexistent_agent", params={})],
            timeout=30,
        )

        inputs = {}

        with patch("main.send_safepoint") as mock_safepoint, patch("main.call_agent_via_kordp") as mock_call_agent:
            mock_safepoint.return_value = True
...
class TestValidation:
    ...
    def test_invalid_workflow_name(self):
        ...
        with pytest.raises(Exception):
            WorkflowDefinition(name="", steps=[])  # Leerer Name

    def test_step_timeout_limits(self):
        """Test Step-Timeout Grenzen"""
        # Gültiger Timeout
        step = StepDefinition(name="test", action="test", timeout=30)
        assert step.timeout == 30

        # Ungültige Timeouts werden durch Pydantic validiert
        with pytest.raises(ValueError):
            StepDefinition(name="test", action="test", timeout=1)  # Zu niedrig (< 5)

        with pytest.raises(ValueError):
            StepDefinition(name="test", action="test", timeout=500)  # Zu hoch (> 300)

    def test_retry_count_limits(self):
        """Test Retry-Count Grenzen"""
        # Gültiger Retry-Count
        step = StepDefinition(name="test", action="test", retry_count=3)
        assert step.retry_count == 3

        # Ungültige Retry-Counts
        with pytest.raises(ValueError):
            StepDefinition(name="test", action="test", retry_count=-1)  # Negativ

        with pytest.raises(ValueError):
            StepDefinition(name="test", action="test", retry_count=10)  # Zu hoch (> 5)
...
class TestHealthCheck:
    ...
    async def test_health_response_structure(self):
        ...
        with (
            patch("main.check_portier_connection") as mock_portier,
...
async def test_create_workflow():
    ...
    workflow_def = {
        ...
        "description": "Test Workflow",
        "steps": [{"name": "step1", "action": "call_agent", "agent": "opena3", "params": {"query": "test"}}],
        "timeout": 60,
    }
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{BASE_URL}/workflows/create", json=workflow_def, headers=headers, timeout=5.0)
...
async def test_list_workflows():
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{BASE_URL}/workflows/list", headers=headers, timeout=5.0)
...
async def test_execute_workflow_sync():
    ...
    execute_request = {"workflow_name": "demo_multi_agent", "inputs": {"test_param": "test_value"}, "mode": "sync"}

    headers = {"Authorization": f"Bearer {BEARER_TOKEN}"}

    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{BASE_URL}/workflows/execute", json=execute_request, headers=headers, timeout=10.0
        )
...
async def test_execute_workflow_not_found():
    ...
    execute_request = {"workflow_name": "non_existent_workflow", "inputs": {}, "mode": "sync"}

    headers = {"Authorization": f"Bearer {BEARER_TOKEN}"}

    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{BASE_URL}/workflows/execute", json=execute_request, headers=headers, timeout=5.0
        )
...
async def test_workflow_status():
    ...
    execute_request = {"workflow_name": "demo_multi_agent", "inputs": {}, "mode": "sync"}
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Status abfragen
        status_response = await client.get(f"{BASE_URL}/workflows/status/{workflow_id}", headers=headers, timeout=5.0)
...
async def test_invoke_execute_workflow():
    ...
    invoke_request = {"action": "execute_workflow", "params": {"workflow_name": "demo_multi_agent", "inputs": {}}}

    headers = {"Authorization": f"Bearer {BEARER_TOKEN}"}

    async with httpx.AsyncClient() as client:
        response = await client.post(f"{BASE_URL}/invoke", json=invoke_request, headers=headers, timeout=10.0)
...
async def test_invoke_get_status():
    ...
    # Erst Workflow ausführen
    exec_request = {"action": "execute_workflow", "params": {"workflow_name": "demo_multi_agent", "inputs": {}}}

    headers = {"Authorization": f"Bearer {BEARER_TOKEN}"}

    async with httpx.AsyncClient() as client:
        # Workflow ausführen
        exec_response = await client.post(f"{BASE_URL}/invoke", json=exec_request, headers=headers, timeout=10.0)

        workflow_id = exec_response.json()["result"]["workflow_id"]

        # Status via invoke abfragen
        status_request = {"action": "get_status", "params": {"workflow_id": workflow_id}}

        status_response = await client.post(f"{BASE_URL}/invoke", json=status_request, headers=headers, timeout=5.0)
...
async def test_auth_required():
    ...
    async with httpx.AsyncClient() as client:
        # Ohne Token
        response = await client.get(f"{BASE_URL}/workflows/list", timeout=5.0)
...
async def test_strict_json_validation():
    ...
    workflow_def = {"name": "test_strict", "steps": [], "timeout": 60, "invalid_field": "should_fail"}  # Extra field
...
async def test_full_workflow_lifecycle():
    ...
    async with httpx.AsyncClient() as client:
        ...
        workflow_def = {
            ...
            "steps": [{"name": "step1", "action": "call_agent", "agent": "opena3", "params": {}}],
            "timeout": 60,
        }

        create_response = await client.post(
            f"{BASE_URL}/workflows/create", json=workflow_def, headers=headers, timeout=5.0
        )
        assert create_response.status_code == 200

        # 2. Workflow ausführen
        execute_request = {"workflow_name": workflow_def["name"], "inputs": {}, "mode": "sync"}
        ...
        # 4. In Liste prüfen
        list_response = await client.get(f"{BASE_URL}/workflows/list", headers=headers, timeout=5.0)
