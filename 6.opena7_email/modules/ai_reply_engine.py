...
class AIReplyEngine:
    ...
    async def handle_specialized(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": "ai_processing_failed", "message": str(e), "timestamp": datetime.now().isoformat()}
        ...
        try:
            ...
            if self.available and self.client:
                ...
                response = await self._call_openai(
                    prompt=self._build_reply_prompt(email_text, context, tone, language), max_tokens=500
                )
    ...
    async def _classify_email(self, payload: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if self.available and self.client:
                # Real classification
                response = await self._call_openai(
                    prompt=self._build_classification_prompt(email_text, subject), max_tokens=200
                )
            ...
            return {"status": "success", "classification": classification, "timestamp": datetime.now().isoformat()}
    ...
    async def _extract_info(self, payload: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if self.available and self.client:
                # Real extraction
                response = await self._call_openai(
                    prompt=self._build_extraction_prompt(email_text, fields), max_tokens=300
                )
    ...
    async def _sentiment_analysis(self, payload: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if self.available and self.client:
                ...
                response = await self._call_openai(prompt=self._build_sentiment_prompt(email_text), max_tokens=150)
            ...
            return {"status": "success", "sentiment": sentiment, "timestamp": datetime.now().isoformat()}
    ...
    async def _priority_score(self, payload: dict) -> dict[str, Any]:
        ...
        try:
            ...
            if self.available and self.client:
                ...
                response = await self._call_openai(
                    prompt=self._build_priority_prompt(email_text, subject, sender), max_tokens=100
                )
            ...
            return {"status": "success", "priority": priority, "timestamp": datetime.now().isoformat()}
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an intelligent email assistant. Provide accurate, helpful responses in the requested format.",
                    },
            ...
            return {"content": f"API Error: {e!s}"}
