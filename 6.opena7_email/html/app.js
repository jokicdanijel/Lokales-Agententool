...
async function runCommand() {
    ...
    try {
        ...
        log('cmd_output', `🚀 Executing email command: ${payload.command || 'unknown'}`, true);
...
async function runSpecialized() {
    ...
    try {
        const payload = JSON.parse(payloadText);
        log('spec_output', `🤖 Executing AI function: ${payload.action || 'unknown'}`, true);
...
function loadAITemplate(type) {
    const templates = {
        generate_reply: {
            ...
            email_text: "Hallo, ich habe eine Frage zu Ihrem Service. Können Sie mir bitte weiterhelfen?",
            tone: "professional",
            language: "german"
        },
        classify_email: {
        ...
        extract_info: {
            action: "extract_info",
            email_text: "Hi, my name is John Doe from Acme Corp. I need help with project deadline on Friday.",
            fields: ["contact", "company", "intent", "deadline"]
        },
        sentiment_analysis: {
            action: "sentiment_analysis",
            email_text: "I am very disappointed with your service. This is unacceptable!"
        },
        priority_score: {
...
async function loadMetrics() {
    try {
        ...
        updateMetricDisplay('emails_processed', 
            (response.statistics?.emails_received || 0) + (response.statistics?.emails_sent || 0));
        updateMetricDisplay('ai_replies', response.statistics?.ai_replies_generated || 0);
        updateMetricDisplay('uptime', response.uptime_formatted || '--');
        updateMetricDisplay('success_rate', 
            response.performance ? `${100 - response.performance.error_rate_percent}%` : '--');
    ...
    try {
        ...
        if (response.logs && Array.isArray(response.logs)) {
            ...
            response.logs.forEach(logEntry => {
                log('logs_output', `[${logEntry.timestamp}] ${logEntry.level}: ${logEntry.message}`);
            });
        } else {
...
async function loadConfig() {
    try {
        ...
        log('config_output', `❌ Configuration loading failed: ${error.message}`, true);
