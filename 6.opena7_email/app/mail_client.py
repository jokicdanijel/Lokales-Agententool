...
from .models import (
    MailMessage, AttachmentInfo, MailRunResponse, MailErrorResponse,
    MailAction, SentimentType, EventLog, Safepoint
)
...
class AttachmentHandler:
    ...
    async def process_attachment(self, filename: str, content: bytes, msg_id: str) -> Optional[AttachmentInfo]:
...
class MailClient:
    ...
    async def connect_imap(self) -> bool:
        ...
        try:
            ...
            self.imap_conn = aioimaplib.IMAP4_SSL(
                config.MAIL_IMAP_HOST,
                config.MAIL_IMAP_PORT
            )
    ...
    async def _fetch_single_message(self, msg_id: bytes) -> Optional[MailMessage]:
            ...
            for part in msg.walk():
                if part.get_content_disposition() == "attachment":
                    ...
                    if filename:
                        ...
                        att_info = await self.attachment_handler.process_attachment(
                            filename, content, msg_id.decode()
                        )
                        if att_info:
    ...
    async def send_message(self, to: str, subject: str, body_text: str, 
