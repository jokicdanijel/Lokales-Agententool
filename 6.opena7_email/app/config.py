...
class OpenaConfig(BaseSettings):
    ...
    # Security & Filtering
    MAIL_ALLOWLIST: list[str] | None = None
    MAIL_BLOCKLIST: list[str] | None = None

    @field_validator("MAIL_ALLOWLIST", mode="before")
    @classmethod
    def validate_mail_allowlist(cls, v):
        if isinstance(v, str):
            return [s.strip() for s in v.split(",") if s.strip()]
        return v or []

    @field_validator("MAIL_BLOCKLIST", mode="before")
    @classmethod
    def validate_mail_blocklist(cls, v):
        if isinstance(v, str):
            return [s.strip() for s in v.split(",") if s.strip()]
        return v or []
    ...
    DANGEROUS_EXTENSIONS: list[str] = [".exe", ".dll", ".zip", ".rar", ".bat", ".cmd", ".scr", ".vbs", ".js"]
