...
async def specialized(payload: dict[str, Any], authorized: bool = Depends(verify_token)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Specialized action failed: {e!s}")
...
class InboxListRequest(BaseModel):
    ...
    search_criteria: str | None = Field(None, description="IMAP search criteria (e.g., 'UNSEEN')")
...
def mask_secrets(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: (
                "***"
                if any(s in k.lower() for s in ["token", "password", "secret", "key", "bearer", "auth", "credential"])
                else mask_secrets(v)
            )
...
def get_imap_connection():
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"IMAP connection failed: {e!s}")
...
def get_smtp_connection():
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"SMTP connection failed: {e!s}")
...
async def inbox_list(req: InboxListRequest, _: bool = Depends(verify_token)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list inbox: {e!s}")
...
async def folders_list(_: bool = Depends(verify_token)):
    ...
    try:
        ...
        result = {"count": len(folder_list), "folders": folder_list}
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list folders: {e!s}")
...
async def command(req: CommandRequest, _: bool = Depends(verify_token)):
    ...
    write_safepoint("emailp", "kordp", "CMD", {"command": req.command, "params": mask_secrets(req.params)}, request_id)
