...
def log_test(test_num: int, test_name: str, passed: bool, details: str = ""):
    ...
    print(
        f"{color}[{status}] Test {test_num} ({test_name}): {'PASS' if passed else 'FAIL'}{reset}{f' - {details}' if details else ''}"
    )
...
def test_6_create_category():
    ...
    try:
        payload = {"name": "Test Category", "description": "Integration test category"}
...
def test_8_update_inventory():
    ...
    try:
        payload = {"sku": "TEST-001", "quantity": 100, "warehouse": "main"}
...
def test_13_delete_product():
    ...
    try:
        ...
        resp = requests.delete(
            f"{BASE_URL}/products/delete", headers=HEADERS, params={"product_id": product_id}, timeout=5
        )
...
def test_14_strict_json():
    ...
    try:
        ...
        payload = {"title": "Test", "sku": "TEST-STRICT", "price": 10.0, "unknown_field": "should_be_rejected"}
...
def main():
    ...
    print("=" * 70)
    print(
        f"Total: {total_tests}, Passed: {passed_tests}, Failed: {failed_tests}, Rate: {(passed_tests/total_tests*100):.1f}%"
    )
