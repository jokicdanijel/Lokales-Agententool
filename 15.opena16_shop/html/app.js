class ShopDashboard {
    ...
    bindEvents() {
        document.getElementById('product-form')?.addEventListener('submit', e => this.createProduct(e));
    ...
    async loadProducts() {
        const data = await this.apiCall(CONFIG.ENDPOINTS.PRODUCTS_LIST, 'POST', { max_results: 100 });
        if (data?.products) {
    ...
    async loadOrders() {
        const data = await this.apiCall(CONFIG.ENDPOINTS.ORDERS_LIST, 'POST', { max_results: 50 });
        if (data?.orders) {
    ...
    async createProduct(e) {
        ...
        const res = await this.apiCall(CONFIG.ENDPOINTS.PRODUCTS_CREATE, 'POST', product);
        if (res?.product_id) {
    ...
    async createOrder(e) {
        ...
        if (!productSku) { this.toast('Bitte Produkt wählen', 'warning'); return; }
        const order = {
            customer_name: document.getElementById('order-customer').value,
            customer_email: document.getElementById('order-email').value,
            items: [{ sku: productSku, quantity: parseInt(document.getElementById('order-quantity').value) || 1 }],
            shipping_address: document.getElementById('order-address').value,
            currency: CONFIG.CURRENCY
        };
        const res = await this.apiCall(CONFIG.ENDPOINTS.ORDERS_CREATE, 'POST', order);
        if (res?.order_id) {
    ...
    renderProducts() {
        ...
        if (!this.products.length) { container.innerHTML = '<p class="empty-state">Keine Produkte</p>'; return; }
        container.innerHTML = this.products.slice(0, 20).map(p => `
    ...
    renderOrders() {
        ...
        if (!this.orders.length) { container.innerHTML = '<p class="empty-state">Keine Bestellungen</p>'; return; }
        container.innerHTML = this.orders.slice(0, 15).map(o => `
    ...
    updateProductSelect() {
        ...
        select.innerHTML = '<option value="">-- Produkt wählen --</option>' +
            this.products.filter(p => p.status === 'active').map(p => `<option value="${p.sku}">${p.title} (€${p.price})</option>`).join('');
    }
    updateMetrics() {
        ...
        const avgPrice = this.products.length ? this.products.reduce((s, p) => s + (p.price || 0), 0) / this.products.length : 0;
...
function saveToken() {
    ...
    if (token) { localStorage.setItem(CONFIG.AUTH.TOKEN_KEY, token); shop.toast('Token gespeichert', 'success'); }
}
