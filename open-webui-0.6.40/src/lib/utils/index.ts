...
export const processDetails = (content) => {
  ...
  const detailsRegex =
    /<details\s+type="tool_calls"([^>]*)>([\s\S]*?)<\/details>/gis;
    ...
    switch (schemaRef.type) {
      case "object":
        ...
        for (const [propName, propSchema] of Object.entries(
          ...
          schemaObj.properties[propName] = resolveSchema(
            propSchema,
            components,
          );
        }
...
export const extractContentFromFile = async (file: File) => {
  ...
  try {
    ...
    throw new Error(
      "Unsupported or non-text file type: " + (file.name || type),
    );
  }
};
