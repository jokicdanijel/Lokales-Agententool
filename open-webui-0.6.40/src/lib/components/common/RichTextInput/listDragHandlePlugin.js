...
export function listDragHandlePlugin(options = {}) {
      ...
      if (isListNode(node)) {
        ...
        node.content.forEach((li) => {
          normalizedItems.push(
            normalizeItemForList(state, li, parentTargetListNode),
          );
        });
    ...
    if (wantedType.attrs) {
      for (const key in wantedType.attrs) {
        if (Object.prototype.hasOwnProperty.call(itemNode.attrs || {}, key)) {
          ...
          newAttrs[key] =
            typeof spec?.default !== "undefined" ? spec.default : null;
        }
      }
    }
  ...
  const apply = (tr, prev) => {
    ...
    if (meta) {
      ...
      if (meta.type === "clear")
        next = { ...next, dragging: null, dropTarget: null };
    }
    return next;
  };
  ...
  const setDrag = (view, dragging) =>
    view.dispatch(
      view.state.tr.setMeta(listPointerDragKey, { type: "set-drag", dragging }),
    );
  const setDrop = (view, drop) =>
    view.dispatch(
      view.state.tr.setMeta(listPointerDragKey, { type: "set-drop", drop }),
    );
  const clearAll = (view) =>
  ...
  function moveItem(view, fromStart, toPos) {
    ...
    const deleteFrom =
      parentList.childCount === 1 ? parentListStart : fromStart;
    const deleteTo =
    ...
    const nodeToInsert = listAtDest
      ? normalizeItemForList(state, orig, listAtDest.node)
      : orig;
    ...
    try {
      ...
      if (wrapName) {
        ...
        if (wrapType) {
          const frag = wrapType.create(
            null,
            normalizeItemForList(state, orig, wrapType),
          );
  ...
  const updateGhost = (ghost, dx, dy) => {
    if (ghost)
      ghost.style.transform = `translate(${Math.round(dx)}px, ${Math.round(dy)}px)`;
  };
