  ...
  if (connections && !base) {
    ...
    if (connections) {
      ...
      for (const idx in responses) {
        ...
        let models = Array.isArray(response)
          ? response
          : (response?.data ?? []);
  ...
  try {
    ...
    const matchingRoute = Object.entries(serverData.openapi.paths).find(
      ([_, methods]) =>
        Object.entries(methods as any).some(
          ([__, operation]: any) => operation.operationId === name,
        ),
    );
    ...
    if (operation.requestBody && operation.requestBody.content) {
      ...
      if (params !== undefined) {
        ...
        throw new Error(
          `Request body expected for operation '${name}' but none found.`,
        );
      }
    }
...
export const generateFollowUps = async (
  ...
  const res = await fetch(
    `${WEBUI_BASE_URL}/api/v1/tasks/follow_ups/completions`,
    {
      method: "POST",
      headers: {
        Accept: "application/json",
...
export const getPipelineValvesSpec = async (
  token: string,
  pipeline_id: string,
  urlIdx: string,
) => {
