            ...
            if tool_id.startswith("server:"):
                ...
                if type == "openapi":
                    ...
                    function_name_filter_list = (
                        tool_server_connection.get("config", {}).get("function_name_filter_list", "").split(",")
                    )
...
async def get_tool_servers_data(servers: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    results = []
    for (id, idx, server, url, info, _), response in zip(server_entries, responses, strict=False):
